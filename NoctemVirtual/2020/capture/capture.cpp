...
void solve() {
	...
	for (int i=0; i<R; i++) {
		for (int j=0; j<C; j++) {
			for (int k=0; k<N; k++) {
				chmn(dist[i][j], (ll)(abs(i - net[k].FI) + abs(j - net[k].SE)));
			}
		}
	}
//	debug2(dist, R, C);
	...
	for (int i=0; i<R; i++) {
		multiset<ll> cur;
		for (int j=0; j<C; j++) {
			for (int k=max(0LL, i-A+1); k<=i; k++) {
				cur.INS(dist[k][j]);
				if (j >= B) {
					cur.erase(cur.find(dist[k][j-B]));
				}
			}
			if (!cur.empty() && cur.size() == A*B) {
				chmx(ans, *cur.begin());
			}
		}
	}
