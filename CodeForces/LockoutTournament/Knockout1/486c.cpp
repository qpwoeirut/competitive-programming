//486c.cpp created at 05/01/22 21:37:12
...
void solve(int testcase) {
    ...
    const int lo1 = *diff1.begin(), hi1 = *diff1.rbegin();
    const int move1 = hi1 - lo1 + min(abs(K - lo1), abs(K - hi1));
    const int lo2 = *diff2.begin(), hi2 = *diff2.rbegin();
    const int move2 = hi2 - lo2 + min(abs(K - lo2), abs(K - hi2));
    
    ll ans = min(move1, move2);

    for (const int i: diff1) {
