...
void dfs(int node, int prev) {
	...
	for (set<int>::iterator it=adj[node].begin(); it!=adj[node].end(); it++) {
		if (*it != prev) dfs(*it, node);
	}
	sub[node].SE = ctr-1;
}
bool is_par(int n1, int n2) {
	return sub[n1].FI <= sub[n2].FI && sub[n2].SE <= sub[n1].SE;
}

inline const bool cmp_sub(int a, int b) {
	return (sub[a].SE - sub[a].FI) < (sub[b].SE - sub[b].FI);
}
...
//		debug(K);
		for (int j=0; j<K; j++) {
			cin >> queries[j];
			queries[j]--;
			queries[j] = par[queries[j]];
		}
//		debug1(queries, K);

		int dest = *min_element(queries, queries+K, cmp_sub);
//		debug(dest);


		bool ok = true;
		for (int k=0; k<K; k++) {
			if (!is_par(queries[k], dest)) {
				ok = false;
			}
		}
