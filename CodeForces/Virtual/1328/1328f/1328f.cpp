...
void solve() {
	...
	ll lc = 0, hc = 0;
	ll lowest = ct.begin()->FI + 1;
	for (mll::iterator it=ct.begin(); it!=ct.end(); it++) {
		hc += (it->FI - lowest) * it->SE;
	}

	ll lower = 0, higher = N;

	ll ans = 1e18;
	ll prev = ct.begin()->FI - 1;
	for (mll::iterator it=ct.begin(); it!=ct.end(); it++) {
		if (it->SE == K) {
			ans = 0;
			break;
		...
		hc -= higher * (it->FI - prev - 1);
		higher -= it->SE;

		chmn(ans, lc + hc + K - it->SE);
		if (lower + it->SE >= K) {
			chmn(ans, lc + K - it->SE);
		}
		if (higher + it->SE >= K) {
