//1400b.cpp created at 01/28/22 17:29:34
...
void solve(int testcase) {
    ...
    ll best = 0;
    for (int pcs=0; pcs<=cs && pcs * s <= p; ++pcs) {
        ll fcs = min(cs - pcs, f / s);
        ll pp = p - pcs*s, ff = f - fcs*s;

        ll pcw = min(cw, pp / w);
        ll fcw = min(cw - pcw, ff / w);

        chmx(best, pcs + pcw + fcs + fcw);
    }
    cout << best << '\n';
}
