//1672f.cpp created at 04/23/22 08:42:50
...
void solve(int testcase) {
    ...
    int mover = A[0];
    for (int i=1; i<=N; ++i) {
        if (pos[i].size() > 0 && (pos[mover].size() < pos[i].size() || (pos[mover].size() == pos[i].size() && mover < i))) {
            mover = i;
        }
    }
    //debug1(pos, N+1);

    assert(pos[mover].size() > 0);
    int mpos = pos[mover].back();
    pos[mover].pop_back();

    priority_queue<pii> pq;
    for (int i=1; i<=N; ++i) {
        if (pos[i].size() > 0) pq.emplace(pos[i].size(), i);
    }

    pii holding(0, -1);
    while (pq.size() > 0) {
        const pii cur = pq.top(); pq.pop();
        const int val = cur.se;
        //debug(cur, val, mpos);
        //debug1(A, N);
        swap(A[mpos], A[pos[val].back()]);
        mpos = pos[val].back();
        pos[val].pop_back();

        if (holding.fi > 0) pq.push(holding);
        holding = pii(pos[val].size(), val);
    }
