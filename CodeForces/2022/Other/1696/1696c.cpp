//1696c.cpp created at 06/24/22 23:31:16
...
void solve(int testcase) {
    ...
    for (int i=0; i<N; ++i) {
        ...
        if (!expand.empty() && expand.back().fi == x) expand.back().se += ct;
        else expand.eb(x, ct);
    }
    for (int i=0; i<M; ++i) {
        if (expand.empty()) {
        ...
        ll ct = 1, x = B[i];
        while (x > expand.front().fi && x % K == 0) {
            x /= K;
            ct *= K;
        }
        if (x != expand.front().fi) {
            cout << "No\n";
            return;
        }
        expand.front().se -= ct;
        if (expand.front().se < 0) {
            cout << "No\n";
            return;
        }
        if (expand.front().se == 0) expand.pop_front();
        debug(expand);
    }
    cout << (expand.empty() ? "Yes\n" : "No\n");
}
