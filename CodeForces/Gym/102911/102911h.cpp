//102911h.cpp created at 01/01/21 19:41:34
...
void solve(int testcase) {
    ...
    for (int i=0; i<N; ++i) {
        cin >> A[i].FI;
        nums.push_back(A[i].FI);
    }
    sort(all(nums));

    for (int i=0; i<N; ++i) {
        A[i].FI = (lower_bound(all(nums), A[i].FI) - nums.begin()) + 1;
        cin >> A[i].SE;
    }
    
    ll ans = 0;
    for (int i=N-1; i>=0; --i) {
        debug(i, A[i], query(A[i].FI));
        ans += query(A[i].FI) * A[i].SE;
        add(A[i].FI, A[i].SE);
    }
