...
struct Item {
	...
	Item (int _r, int _c, ll _h, ll _dist) {
		r = _r;
		c = _c;
		h = _h;
		dist = _dist;
...
ll f(ll start_h) {
	...
	while (pq.size() > 0) {
		...
		for (int i=0; i<2; i++) {
			...
			nxt.h++;
			if (nxt.h < grid[nxt.r][nxt.c]) {
				nxt.dist += grid[nxt.r][nxt.c] - nxt.h;
			}
			else if (nxt.h > grid[nxt.r][nxt.c]) {
				ll up = cur.r + cur.c + 1;
				nxt.dist += up * (nxt.h - grid[nxt.r][nxt.c]);

			}

			if (V[nxt.r][nxt.c] == false) pq.push(nxt);
			V[nxt.r][nxt.c] = true;
		}
	}
