...
//	debug(node);
	bool ret = false;
	if (from[node].find(par) == from[node].end()) {
		from[node].INS(par);
		cur[node] = true;
		for (set<pii>::iterator it=adj[node].begin(); it!=adj[node].end();) {
			if (it->FI == par) {
				++it;
				continue;
			}
			bool erased = false;
			if (cur[it->FI] || dfs(it->FI, node)) {
//				debug(*it);
				if (it->SE == 0) {
					adj[it->FI].erase(pii(node, 0));
					adj[it->FI].INS(pii(node, 1));
...
void solve() {
	...
	fill(cur, cur+N, false);
	fill(from, from+N, set<int>());
...
//	debug(adj);
	...
	cout << "YES" << endl;
	for (int i=0; i<N; i++) {
		for (set<pii>::iterator it=adj[i].begin(); it!=adj[i].end(); it++) {
			if (it->SE != 0 || i < it->FI) {
				cout << i+1 << ' ' << it->FI+1 << endl;
			}
		}
	}
}
