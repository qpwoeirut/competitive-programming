...
void solve(int t) {
    ...
    assert(sz(ans) == N);
    for (int gap=1; gap<=Y-X; gap++) {
        if ((Y-X) % gap != 0) continue;
        deque<int> cur;
        for (int i=0; i<N || cur.empty() || cur.back() < Y; i++) {
            cur.PB(((X+gap-1) % gap) + 1 + i * gap);
            if (cur.size() > N) {
                cur.pop_front();
            }
        }
        assert(sz(cur) == N);
        if (cur.front() <= X && Y <= cur.back() && cur.back() < ans.back()) {
            ans = cur;
        }
    }
