...
void inc(ll BIT[], int idx) {
	for (; idx<MN; idx |= idx + 1) ++BIT[idx];
}
...
//	debug1(ctb, M+1);
	...
	for (int i=N; i>=0; i--) {
		groups += cta[i];
		cta[i] = cta[i+1] + groups;
	}
	groups = 0;
	for (int i=M; i>=0; i--) {
		groups += ctb[i];
		ctb[i] = ctb[i+1] + groups;
	}
//	debug1(cta, N+1);
//	debug1(ctb, M+1);
...
//		debug(ans);
		...
		if (K % i == 0) {
			if (i <= N && K/i <= M) {
//				ans += (ta - sum(BITa, i)) * (tb - sum(BITb, K/i));
				ans += cta[i] * ctb[K/i];
				if (i*i == K) continue;
			}
			if (K/i <= N && i <= M) {
//				ans += (ta - sum(BITa, K/i)) * (tb - sum(BITb, i));
				ans += cta[K/i] * ctb[i];
