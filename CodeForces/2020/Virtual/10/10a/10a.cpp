...
void solve() {
	...
	ll ans = 0;
	for (int i=0; i<N; i++) {
		ans += P[0] * (times[i].SE - times[i].FI);
		if (i > 0) {
			ans += P[0] * min(T[0], times[i].FI - times[i-1].SE);
			ans += P[1] * min(T[1], max(times[i].FI - times[i-1].SE - T[0], 0));
			ans += P[2] * max(0, times[i].FI - times[i-1].SE - T[0] - T[1]);
		}
	}
