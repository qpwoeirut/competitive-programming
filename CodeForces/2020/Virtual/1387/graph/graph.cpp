...
ll dfs(int node) {
	...
	for (set<pii>::iterator it=adj[node].begin(); it!=adj[node].end(); it++) {
		if (val[it->FI] == INIT) {
			val[it->FI] = it->SE - val[node];
			ret += dfs(it->FI);
		}
		else if (val[it->FI] + val[node] != it->SE) {
			flag = true;
		}
		if (flag) {
			return 0;
...
void solve() {
	...
	for (int i=0; i<M; i++) {
		...
		if (adj[u].find(pii(v, mul + (t % (mul+mul)))) != adj[u].end()) dpara = true;
	}
	...
	vector<pll> ans(starts.size(), pll(INIT, INIT));
	for (ll x = -270; x <= 270; x+=5) {
		fill(val, val+N, INIT);
