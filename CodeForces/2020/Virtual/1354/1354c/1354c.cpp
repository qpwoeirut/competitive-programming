...
//	debug(angle * PI / 180);
	...
	double ans;
	if ((N >> 1) & 1) {
		double projAngle = PI / (N * 2);
		double mnX = 1e18, mxX = -1e18, mnY = 1e18, mxY = -1e18;

		for (int i=0; i<N; i++) {
			double projX = cos(2 * PI * i / N + projAngle);
			double projY = sin(2 * PI * i / N + projAngle);
		...
		double mx = max(mxX - mnX, mxY - mnY);
		ans = (mx / sin(PI / N)) / 2;
	}
	else {
		double angle = ((N - 2) * PI) / (N * 2);
		ans = tan(angle);
	}
