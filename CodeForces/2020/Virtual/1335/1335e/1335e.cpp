...
int A[MN];
int ct[205], lf[205], ri[205];
int cct[205], cleft[205], cright[205];
void solve() {
	int N;
	cin >> N;
	...
	int ans = *max_element(ct, ct+204);
	int last = -1;
	for (int i=1; i<=203; i++) {
		copy(ct, ct+205, cct);
		copy(lf, lf+205, cleft);
		copy(ri, ri+205, cright);
		int L = 0, R = N-1;
		++cleft[A[L]]; ++cright[A[R]];
		--cct[A[L]]; --cct[A[R]];
		last = -1;
		while (L < R) {
			if (cleft[i] == cright[i] && cleft[i] != last) {
				last = cleft[i];
				chmx(ans, cleft[i] + *max_element(cct, cct+204) + cright[i]);
			}
