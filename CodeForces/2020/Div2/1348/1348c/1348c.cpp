...
struct cmp {
	inline const bool operator()(const int& a, const int& b) {
		for (int i=0; i<ans[a].size() && i<ans[b].size(); i++) {
...
inline const bool lex_cmp(const string& a, const string& b) {
...
void solve() {
	...
	int si = 0;
	for (si=0; si<K; si++) {
		assert(ans[idx].empty());
		ans[idx].PB(S[si]);
		idx = (idx + 1) % K;
	}
	bool diff = ans[0].back() != ans.back().back();
	assert(si == K);
	assert(idx == 0);
	for (; si<N; si++) {
		if (S[si] == S.back() && !diff) {
			...
			ans[idx] += S.substr(si, string::npos);
			break;
		}
		idx = (idx + 1) % K;
	}

	cout << *max_element(all(ans), lex_cmp) << endl;
}
