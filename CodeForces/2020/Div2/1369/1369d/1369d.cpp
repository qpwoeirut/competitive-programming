...
//int A[MN];
//pair<__int128, __int128> ans[MN];
//pair<__int128, __int128> mods[MN];
//pair<__int128, __int128> mmods[MN];
pll ans[MN];
pll mods[MN];
pll mmods[MN];
void solve() {
...
int main() {
	...
	for (int i=0; i<MN; i++) {
		ans[i] = mods[i] = mmods[i] = pll(0, 0);
	}

	for (int i=0; i<MN; i++) {
		if (i >= 3) {
			ans[i].SE = (ans[i-2].FI + ans[i-1].FI + ans[i-2].FI) % MOD;
//			mods[i].SE = mods[i-2].FI + mods[i-1].FI + mods[i-2].FI;
//			while (ans[i].SE >= MD) {
//				ans[i].SE -= MD;
//				mods[i].SE++;
//			}
//			mmods[i].SE = mmods[i-2].FI + mmods[i-1].FI + mmods[i-2].FI;
//			while (mods[i].SE >= MMOD) {
//				mods[i].SE -= MMOD;
//				mmods[i].SE++;
//			}
			ll cur = (ans[i-2].SE + ans[i-1].SE + ans[i-2].SE + 1) % MOD;
//			mods[i].FI = mods[i-2].SE + mods[i-1].SE + mods[i-2].SE;
			ans[i].FI = max(cur, ans[i].SE);
//			while (cur >= MD) {
//				cur -= MD;
//				mods[i].FI++;
//			}
//			mmods[i].FI = mmods[i-2].SE + mmods[i-1].SE + mmods[i-2].SE;
//			while (mods[i].FI >= MMOD) {
//				mods[i].FI -= MMOD;
//				mmods[i].FI++;
//			}
//
//			if (mmods[i].FI < mmods[i].SE) {
//				mmods[i].FI = mmods[i].SE;
//				mods[i].FI = mods[i].SE;
//				ans[i].FI = ans[i].SE;
//			}
//			else if (mmods[i].FI == mmods[i].SE) {
//				if (mods[i].FI < mods[i].SE) {
//					mods[i].FI = mods[i].SE;
//					ans[i].FI = ans[i].SE;
//				}
//				else if (mods[i].FI == mods[i].SE) {
//					ans[i].FI = max(cur, ans[i].SE);
//				}
//				else {
//					ans[i].FI = cur;
...
//				ans[i].FI = cur;
//			}
//			chmx(mxm, max(mmods[i].FI, mmods[i].SE));
		}
	}
