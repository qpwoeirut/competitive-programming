#include <bits/stdc++.h>

using namespace std;

#ifdef LOCAL
#include "qpwoeirut/debug.h"
#else
#define debug
#define debug1
#define debug2
#define debug3
#endif

#define PB push_back
#define INS insert

#define FI first
#define SE second
#define sz(obj) ((int)(obj.size()))
#define all(obj) begin(obj), end(obj)

#define LB lower_bound
#define UB upper_bound

typedef long long ll;
typedef pair<int,int> pii;
typedef pair<ll,ll> pll;

typedef unsigned long long ull;
typedef pair<int,bool> pib;
typedef pair<ll,bool> plb;
typedef pair<double,double> pdd;
typedef map<ll,ll> mll;
typedef map<char, int> mci;
typedef map<string,int> msi;
typedef map<string,string> mss;

ll gcd(ll a, ll b) {return b ? gcd(b, a%b) : a;}

template <class T1,class T2> struct cmpf {
	bool rev;
	inline bool operator()(const pair<T1,T2>& a, const pair<T1,T2>& b) const {return (a.first<b.first)^rev;}
	cmpf(bool b=false) {rev=b;}
};
template <class T1,class T2> struct cmps {
	bool rev;
	inline bool operator()(const pair<T1,T2>& a, const pair<T1,T2>& b) const {return (a.second<b.second)^rev;}
	cmps(bool b=false) {rev=b;}
};

ll binpow(const ll& x, const ll& p, const ll& mod) {assert(mod>0);
	if (p == 0) return 1;
	if (p == 1) return x % mod;
	if (p & 1) return (binpow((x*x) % mod, p/2, mod) * x) % mod;
	return binpow((x*x) % mod, p/2, mod) % mod;
}

template <class T> void chmn(T& a, const T& b) {if (a>b) a=b;}
template <class T> void chmx(T& a, const T& b) {if (a<b) a=b;}

void setIO(const string& filename = "") {
	ios_base::sync_with_stdio(false);
	cin.tie(nullptr); cout.tie(nullptr);
}

int chr[8] = {-1, 0, 1, 0, -1, -1, 1, 1};
int chc[8] = {0, 1, 0, -1, -1, 1, -1, 1};

const ll MOD = 1e9+7; //998244353;
const int INIT = 1001001001;
const int MN = 100001;

ll N, M, K;
int A[MN];
//int G[MN][MN];
set<int> adj[MN];

int tsize[MN];
int dfs(int node, int par) {
    int ret = 1;
    for (set<int>::iterator it=adj[node].begin(); it!=adj[node].end(); it++) {
        if (*it == par) continue;
        ret += dfs(*it, node);
    }
    tsize[node] = ret;
    return ret;
}

void solve(int t) {
    cin >> N;

    for (int i=0; i<N; i++) {
        adj[i].clear();
    }

    for (int i=0; i<N-1; i++) {
        int u,v;
        cin >> u >> v;
        --u; --v;

        adj[u].INS(v);
        adj[v].INS(u);
    }
    
    cin >> M;
    vector<ll> factors(M);
    for (int i=0; i<M; i++) {
        cin >> factors[i];
    }
    sort(all(factors));
    while (factors.size() >= N) {
        assert(sz(factors) >= 2);
        ll tmp = factors.back();
        factors.pop_back();
        assert(sz(factors) >= 1);
        factors.back() = (factors.back() * tmp) % MOD;
    }
    reverse(all(factors));

    fill(tsize, tsize+N, 0);
    int total_size = dfs(0, -1);
    assert(total_size == N);

    vector<ll> weights;
    for (int i=0; i<N; i++) {
        for (set<int>::iterator it=adj[i].begin(); it!=adj[i].end(); it++) {
            if (i >= *it) continue;

            ll x = min(tsize[i], tsize[*it]);
            weights.push_back((N-x) * x);
        }
    }
    assert(sz(weights) == N-1);
    sort(all(weights), greater<ll>());

    while (factors.size() < N-1) {
        factors.PB(1);
    }

    ll ans = 0;
    for (int i=0; i<sz(weights); i++) {
        ans += (weights[i] * factors[i]) % MOD;
        ans %= MOD;
    }

    cout << ans << endl;
}
        
        
int main() {
    setIO();
    
    ll T = 1;
    cin >> T;
    
    for (int t=1; t<=T; t++) solve(t);
    
    return 0;
}
