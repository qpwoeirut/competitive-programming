...
//	debug(order);
	...
	bool ok = true;
	vector<int> ans;
	for (map<int,set<int> >::iterator mit=order.begin(); mit!=order.end(); mit++) {
		for (set<int>::iterator it=mit->SE.begin(); it!=mit->SE.end(); it++) {
			if (neighbors[*it].find(mit->FI) != neighbors[*it].end() || consec[*it] + 1 != mit->FI) {
				ok = false;
			}
			else {
				ans.PB(*it);
				for (set<int>::iterator nit=adj[*it].begin(); nit!=adj[*it].end(); nit++) {
					pair<set<int>::iterator,bool> ret = neighbors[*nit].INS(mit->FI);
					set<int>::iterator sit = ret.FI;
					while (sit != neighbors[*nit].end() && consec[*nit] + 1 == *sit) {
						consec[*nit]++;
						sit++;
					}
				}
