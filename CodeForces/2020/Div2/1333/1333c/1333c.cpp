...
int main() {
	...
	for (int i=0; i<N; i++) {
		sum += A[i];
		map<ll,vector<ll> >::iterator it = pos.find(sum);
		if (it == pos.end()) {
			pos[sum] = vector<ll>(1, i+1);
		}
		else {
	...
	vector<pll> bad;
	for (map<ll,vector<ll> >::iterator it=pos.begin(); it!=pos.end(); it++) {
		for (int i=0; i+1<it->second.size(); i++) {
			bad.PB(pii(it->second[i], it->second[i+1]));
		}
	}


	sort(all(bad), cmps<ll,ll>());
	vector<pll> intervals;
	for (int i=0; i<bad.size(); i++) {
		while (intervals.size() > 0 && intervals.back().first >= bad[i].first) {
			intervals.pop_back();
		}
		intervals.PB(bad[i]);
	}
	...
	for (int i=0; i<intervals.size(); i++) {
		ans += ((intervals[i].second - (prevf + 1)) * (intervals[i].second - (prev + 1))) / 2;
		cout << i << " " << to_string(intervals[i]) << " " << prev << " " << ans << endl;
		ans += intervals[i].second - 1 - min(prev>0? prev : INIT, intervals[i].first);
		cout << i << " " << to_string(intervals[i]) << " " << prev << " " << ans << endl;
		prev = intervals[i].second;
		prevf = intervals[i].first;
	}
	ans += (N - intervals.back().second) * ((N - intervals.back().second) + 1) / 2;
