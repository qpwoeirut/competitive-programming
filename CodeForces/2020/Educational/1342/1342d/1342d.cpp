...
struct cmp {
	inline const bool operator()(const int& a, const int& b) {
		...
		return tests[a].size() > tests[b].size();
	}
};
...
void solve() {
	...
	for (int i=0; i<N; i++) {
//		if (pq.size()) cout << tests[pq.top()].size() << " " << C[A[i]] << endl;
		if (pq.empty() || tests[pq.top()].size() >= C[A[i]]) {
			tests.push_back(vector<int>(1, A[i]));
			pq.push(tests.size() - 1);
		}
		else {
			int idx = pq.top();
			pq.pop();
			tests[idx].push_back(A[i]);
			pq.push(idx);
		}
//		debug(tests);
	}
