...
void solve() {
	...
	for (int path=1; path<(1<<N); path++) {
		pii dat = path_data(path);
//		debug(path);
//		debug(dat);
		for (int last=0; last<N; last++) {
			if ((path & (1 << last)) == 0) continue;
			if (1 << (last + 1) >= path) break;
//			debug(last);

			for (int node=0; node<N; node++) {
				if ((path & (1 << node)) == 0) continue;
				if (mat[node][last] == false) continue;
//				debug(node);

				dp[path][last] += dp[path ^ (1 << last)][node];
			}

			if (mat[last][dat.FI] && dat.SE >= 3) {
				ans += dp[path][last];
			}
		}
	}
