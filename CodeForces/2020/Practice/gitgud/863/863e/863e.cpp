...
void solve() {
	...
	for (int i=0; i<N; i++) {
		...
		positions.PB(A[i].FI);
		positions.PB(A[i].SE);
		positions.PB(A[i].FI+1);
		positions.PB(A[i].SE+1);
	}
	sort(all(positions));
	positions.resize((int)(unique(all(positions)) - positions.begin()));
	assert(positions.size() <= N*4);
	for (int i=0; i<N; i++) {
		A[i].FI = LB(all(positions), A[i].FI) - positions.begin();
		A[i].SE = LB(all(positions), A[i].SE) - positions.begin();
