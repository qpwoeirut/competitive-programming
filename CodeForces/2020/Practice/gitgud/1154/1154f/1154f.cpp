...
void solve() {
	...
	for (int i=0; i<M; i++) {
		pii tmp;
		cin >> tmp.FI >> tmp.SE;
		if (tmp.FI > K) continue;
		if (offer.find(tmp.FI) == offer.end()) {
			offer[tmp.FI] = tmp.SE;
		}
		else {
			chmx(offer[tmp.FI], tmp.SE);
		}
	}
	...
	dp[0] = 0;
	for (int i=0; i<=K; i++) {
		chmn(dp[i+1], dp[i] + A[i]);
		for (mii::iterator it=offer.begin(); it!=offer.end(); it++) {
			chmn(dp[i+it->FI], dp[i] + pref[i+it->FI] - pref[i+it->SE]);
		}
	}
//	debug1(A, N);
//	debug1(pref, N+1);
