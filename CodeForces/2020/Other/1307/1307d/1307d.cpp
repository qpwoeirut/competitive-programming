...
int to[MN], from[MN];
inline const bool comp_diff(const int& a, const int& b) {
...
int main() {
	...
	int special[MN];	for (int i=0; i<K; i++) {
		cin >> special[i];
		special[i]--;
	}
	...
	sort(special, special+K, comp_diff);
	int ans = 0;
	int mx = -INIT;
	for (int i=0; i<K; i++) {
		chmx(ans, mx + to[special[i]] + 1);
		chmx(mx, from[special[i]]);
	}
	chmn(ans, to[0]);
