//1856d.cpp created at 08/05/23 09:08:43
...
ll find_max(int L, int R) {
    if (R - L == 1) return L;
    if (R - L == 2) return query(L, R) == 1 ? L : R - 1;

    int mid = (L + R) / 2;
    int a = find_max(L, mid), b = find_max(mid, R);

    int overall = query(a, b+1);
    int less = query(a, b);
    if (overall == less) return b;
    else return a;
}
