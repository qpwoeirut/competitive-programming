//1696e.cpp created at 06/24/22 23:31:18
...
ll choose(ll n, ll k) {
    if (n < k) return 0;
    return (((B[n] * inv[k]) % MOD) * inv[n-k]) % MOD;
}
void solve(int testcase) {
    ...
    ll ans = 0;
    for (int i=0; i<N; ++i) {
        debug(A[i] + i, i + 1, choose(A[i] + i, i + 1));
...
int main() {
    ...
    for (int i=1; i<MN; ++i) {
        B[i] = (i * B[i-1]) % MOD;
        inv[i] = binpow(B[i], MOD-2, MOD);
    }
    debug1(B, 5);debug1(inv, 5);
