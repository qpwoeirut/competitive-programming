...
//		debug1(A, N);
	...
	for (int i=0; i<N; i++) {
		if (A[i] == i || (i == N-1 && A[i] == N)) continue;
//		debug(i);
//		debug(out);
		while (pos[i].size() > 0) {
//			debug1(A, N);
			if (*out.begin() == N && A[N - 1] == i) {
				A[N - 1] = *out.begin();
				ans.PB(N - 1);
			}
			else if (*out.begin() < N && A[*out.begin()] == i) {
				A[*out.begin()] = *out.begin();
				ans.PB(*out.begin());
			}
			else if (*pos[i].begin() < *out.begin()) {
				A[*pos[i].begin()] = *out.begin();
				ans.PB(*pos[i].begin());
			}
			else {
				A[*pos[i].rbegin()] = *out.begin();
				ans.PB(*pos[i].rbegin());
			}
			pos[i].erase(ans.back());
			pos[A[ans.back()]].INS(ans.back());
			out.erase(out.begin());
		}
//		debug1(A, N);
		pos[A[i]].erase(i);
