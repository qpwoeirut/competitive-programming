...
struct cmp {
	inline const bool operator() (const int& a, const int& b) {
		int az = (A[fav[a].FI] == 0) + (A[fav[a].SE] == 0);
		int bz = (A[fav[b].FI] == 0) + (A[fav[b].SE] == 0);
		if (az != bz) {
			return az < bz;
		}

		int ao = (want[fav[a].FI] <= A[fav[a].FI]) + (want[fav[a].SE] <= A[fav[a]].SE);
		int bo = (want[fav[b].FI] <= A[fav[b].FI]) + (want[fav[b].SE] <= A[fav[b]].SE);
		if (ao != bo) {
			return ao < bo;
		}
