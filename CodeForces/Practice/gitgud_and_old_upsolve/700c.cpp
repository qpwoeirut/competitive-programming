//700c.cpp created at 05/14/21 09:49:19
...
struct Edge {
    ...
    Edge(const ll _u, const ll _v, const ll _cost, const int _idx) {
        u = _u;
        v = _v;
        cost = _cost;
        idx = _idx;
    ...
    inline const bool operator<(const Edge& o) const {
        return v < o.v || (v == o.v && (cost < o.cost || (cost == o.cost && idx < o.idx)));
    }
};
...
bool bfs() {
    ...
    for (int i=0; i<q_sz; ++i) {
        debug(i, q[i]);
        for (const Edge& e: adj[q[i]]) {
            if (par[e.v] == -1 && e.flow > 0) {
                par[e.v] = e.idx;
                q[q_sz++] = e.v;
...
void solve(int testcase) {
    ...
    for (int i=0; i<M; ++i) {
        ...
        cin >> u >> v >> w;
        --u; --v;
        adj[u].emplace_back(u, v, w, i+1);
        adj[v].emplace_back(v, u, w, i+1);
    }
    for (int i=0; i<N; ++i) {
        sort(all(adj[i]));
    }
    ...
    for (C=0; C<=2 && bfs(); ++C) {
        for (int u=sink; u!=src; u=par[u]) {
            LB(all(adj[u]), Edge(u, par[u], 0, 0))->used = true;
            LB(all(adj[par[u]]), Edge(par[u], u, 0, 0))->used = true;
        }
    }
    debug(C);
}
