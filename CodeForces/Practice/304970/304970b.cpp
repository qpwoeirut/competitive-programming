//304970b.cpp created at 11/25/20 07:33:37
...
void dfs(const int node, const int par) {
    ...
    for (auto it=adj[node].begin(); it!=adj[node].end(); ++it) {
        ...
        chance[*it] = chance[node] / (double)(sz(adj[node]) - (par != -1));
        dfs(*it, node);
        is_leaf = false;
    }
    if (is_leaf) leaf.push_back(node);
}
...
void solve(int testcase) {
    ...
    for (int i=0; i<sz(leaf); ++i) {
        debug(leaf[i], depth[leaf[i]], chance[leaf[i]]);
        ans += depth[leaf[i]] * chance[leaf[i]];
    }
