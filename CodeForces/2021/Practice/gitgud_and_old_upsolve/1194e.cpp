...
void upd(int idx, const int val) {
    for (; idx<=MX; idx+=(idx&-idx)) BIT[idx] += val;
}
ll query(int idx) {
    ll ret = 0;
...
void solve(int testcase) {
    ...
    for (int i=0; i<V; ++i) {
        ...
        for (int j=i+1; j<V; ++j) {
            ...
            const ll hlines = max(0LL, query(min(A[i].p2.y, A[j].p2.y)) - query(max(A[i].p1.y, A[j].p1.y) - 1));
            ans += (hlines * (hlines - 1)) >> 1;
            debug(hlines);
        }
    }
