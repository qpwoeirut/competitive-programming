//1353f.cpp created at 07/15/21 15:12:42
...
void solve(int testcase) {
    ...
    for (int i=0; i<N; ++i) {
        for (int j=0; j<M; ++j) {
            if (i) {
                if (val[i-1][j] >= G[i][j]) {  // make current path lower
                    if (chmn(ans[i][j], ans[i-1][j] + (val[i-1][j] - G[i][j] + 1) * (i + j))) {
                        debug(i, j, "i0");
                        val[i][j] = G[i][j];
                    }
                } else { // make next cell lower
                    if (chmn(ans[i][j], ans[i-1][j] + G[i][j] - val[i-1][j] - 1)) {
                        debug(i, j, "i1");
                        val[i][j] = val[i-1][j] + 1;
                    }
                }
            ...
            if (j) {
                if (val[i][j-1] >= G[i][j]) {
                    if (chmn(ans[i][j], ans[i][j-1] + (val[i][j-1] - G[i][j] + 1) * (i + j))) {
                        debug(i, j, "j0");
                        val[i][j] = G[i][j];
                    }
                } else {
                    if (chmn(ans[i][j], ans[i][j-1] + G[i][j] - val[i][j-1] - 1)) {
                        debug(i, j, "j1");
                        val[i][j] = val[i][j-1] + 1;
                    }
                }
