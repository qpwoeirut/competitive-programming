//995c.cpp created at 07/02/21 22:43:18
...
void solve(int testcase) {
    ...
    for (int i=0; i<N; ++i) {
        int px, py;
        cin >> px >> py;
        A[i] = make_pair(i, pt(px, py));
    }
    stable_sort(A, A+N, [](const pair<int,pt>& a, const pair<int,pt>& b) {
        return arg(a.se) < arg(b.se);
    });

    for (int i=0; i<N; ++i) {
        if (abs(pos + A[i].se) <= abs(pos - A[i].se)) {
            ans[A[i].fi] = 1;
        } else {
            ans[A[i].fi] = -1;
        }
        pos += A[i].se * ans[A[i].fi];
    }
