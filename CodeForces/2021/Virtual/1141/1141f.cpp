//1141f.cpp created at 07/22/21 20:04:52
...
void solve(int testcase) {
    ...
    map<ll, vector<pll>> block;
    for (int i=0; i<N; ++i) {
        ll sum = 0;
        for (int j=i; j<N; ++j) {
            sum += A[j];
            debug(sum, i, j);
            block[sum].emplace_back(i, j);
        }
    }

    vector<pll> best;
    for (auto& x: block) {
        sort(all(x.se), [](const pll& a, const pll& b) {return a.se < b.se || (a.se == b.se && a.fi < b.fi);});
        debug(x.fi, x.se);

        vector<pll> cur;
        for (const pll& p: x.se) {
            if (sz(cur) == 0 || cur.back().se < p.fi) cur.pb(p);
        }
        if (sz(best) < sz(cur)) {
            best = cur;
        }
