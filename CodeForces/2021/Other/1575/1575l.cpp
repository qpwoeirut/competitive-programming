//1575l.cpp created at 10/02/21 08:33:50
...
void upd(int idx, const int val) {
    for (; idx<MN; idx+=(idx&-idx)) chmx(BIT[idx], val);
}
int query(int idx) {
    int ret = 0;
    for (; idx>0; idx-=(idx&-idx)) chmx(ret, BIT[idx]);
    return ret;
}
...
void solve(int testcase) {
    ...
    int ans = 0;
    for (int i=0; i<N; ++i) {
        const int val = A[i].fi;
        const int idx = A[i].se;
        if (idx < val) {
            continue;
        }

        const int new_val = query(idx - val + 1) + 1;
        debug(idx, val, new_val);
...
/*
dp[i] = best where a_i contributes
dp[i] = max(dp[j] where a_i - a_j ≤ i - j and a_j < a_i) + 1
a_i - i ≤ a_j - j
j - a_j ≤ i - a_i
