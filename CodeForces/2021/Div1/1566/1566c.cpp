//1566c.cpp created at 09/12/21 07:41:44
...
int MEX(char a, char b, char c, char d) {
    bool z = a == '0' || b == '0' || c == '0' || d == '0';
    bool o = a == '1' || b == '1' || c == '1' || d == '1';
    if (z && o) return 2;
    if (z) return 1;
    return 0;
}
...
void solve(int testcase) {
    ...
    for (int i=1; i<=N; ++i) {
        A[i] = A[i-1];
        if (i > 1) {
            chmx(A[i], A[i-2] + MEX(S[i-1], S[i-2], T[i-1], T[i-2]));
        }
        chmx(A[i], A[i-1] + MEX(S[i-1], T[i-1]));
    }
