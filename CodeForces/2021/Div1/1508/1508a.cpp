//1508a.cpp created at 04/16/21 07:37:08
...
string sol(const string& s, const string& t, const char c) {
    int i = 0, j = 0;
    string ans = "";
    while (i < N || j < N) {
        while (i < N && s[i] != c) {
...
void solve(int testcase) {
    ...
    assert(half[0].size() != half[1].size());
    if (half[0].size() > half[1].size()) {
        cout << sol(S[half[0][0]], S[half[0][1]], '0') << '\n';
    } else {
        cout << sol(S[half[1][0]], S[half[1][1]], '1') << '\n';
    }
}
