...
inline const bool cmp_weight(const Street& a, const Street& b) {
    if (weight[a.name] == weight[b.name]) return a.length < b.length;
...
void solve(const string& dataset) {
    ...
    for (int i = 0; i < v; i++) {
        ...
        for (const string& s: car[i]) {
            weight[s] += max(1, int(sqrt(20 / car[i].size())));
        }
    }
    ...
    for (int i=0; i<n; i++) {
        ...
        for (const Street& s: radj[i]) {
            if (weight[s.name] > 0) {
                ans[i].emplace_back(s.name, max(1, int(sqrt(weight[s.name]))));
            }
        }
