...
int main() {
    ...
    freopen((dataset + ".txt").c_str(), "r", stdin);
    freopen(("ans_" + dataset + ".txt").c_str(), "w", stdout);
    cin.tie(0)->sync_with_stdio(0);
    
    cin >> d >> n >> m >> v >> f;

    for (int i = 0; i < m; i++) {
        cin >> e[i].start >> e[i].finish >> e[i].name >> e[i].length;
        street[e[i].name] = e[i];
        adj[e[i].start].pb(e[i]);
        radj[e[i].finish].pb(e[i]);

        flip[e[i].finish].pb(e[i].name);
    }
