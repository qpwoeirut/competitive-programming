/*
 * incremental_house_of_pancakes.cpp
 *
...
ll bs(ll start, ll val) {
	ll lo = 0, hi = 1e9;
	while (lo < hi) {
		ll mid = (lo + hi + 1) / 2;
		if (mid*(start+mid-1) <= val) {
			lo = mid;
		}
		else {
			hi = mid-1;
...
//		debug(lx);
		...
		L -= (lx*(start+start+(2*(lx - 1))))/2;

		start++;
		rx = bs(start, R);
//		debug(rx);

		R -= (rx*(start+start+(2*(rx-1))))/2;
	}
	else {
		rx = bs(start, R);
//		debug(rx);
