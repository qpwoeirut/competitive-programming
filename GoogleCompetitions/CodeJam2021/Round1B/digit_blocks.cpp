//digit_blocks.cpp created at 04/25/21 10:19:46
...
bool solve(int testcase) {
    ...
    for (int i=0; i<N*B; ++i) {
        ...
        int val = -1;
        for (int j=0; j<N; ++j) {
            if (A[j] < B - sub && chmx(val, A[j])) ans = j;
        }
        if (ans == -1) {
            val = B;
            for (int j=0; j<N; ++j) {
                if (A[j] != -1 && chmn(val, A[j])) ans = j;
            }
        }
        assert(ans != -1);
...
int main() {
    ...
    ll TEST_COUNT = 1;
    cin >> TEST_COUNT >> N >> B >> P;
    
    for (int test_case=1; test_case<=TEST_COUNT; ++test_case) {
        if (!solve(test_case)) return 0;
    }
