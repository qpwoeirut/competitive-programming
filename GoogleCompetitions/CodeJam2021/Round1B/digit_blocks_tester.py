...
def RunCase(input_fn, num_towers, height):
  ...
  print(' '.join(towers), file=sys.stderr)
...
def RunCases(num_cases, num_towers, height, need_correct):
  ...
  print("total   :", total_score, file=sys.stderr)
  print("required:", need_correct, file=sys.stderr)
  print("best pos:", int(S_NUMERATOR/S_DENOMINATOR * NUM_CASES), file=sys.stderr)
