//subtransmutation.cpp created at 04/25/21 09:44:36
...
bool check(const int x) {
    ...
    for (int i=x; i>0; --i) {
        ...
        const int a = i - X, b = i - Y;
        if (a > 0) extra[a] = min(extra[a] + extra[i], INF);
        if (b > 0) extra[b] = min(extra[b] + extra[i], INF);
    }
