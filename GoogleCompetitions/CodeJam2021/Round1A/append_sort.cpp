//append_sort.cpp created at 04/09/21 18:00:32
...
bool all_nine(const string& s, const int start) {
...
void solve(int testcase) {
    ...
    for (int i=1; i<N; ++i) {
        if (cmp(A[i-1], A[i])) {
            if (!prefix(A[i-1], A[i]) || all_nine(A[i-1], sz(A[i]))) {
                while (cmp(A[i-1], A[i])) {
                    A[i].push_back('0');
                    ++ans;
                }
