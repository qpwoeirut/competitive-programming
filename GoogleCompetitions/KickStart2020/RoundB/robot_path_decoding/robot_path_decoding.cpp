...
pll eval() {
	pll ret(0, 0);
	for (; idx<prog.size(); idx++) {
		if (prog[idx] == 'N') ret.second--;
		else if (prog[idx] == 'S') ret.second++;
		else if (prog[idx] == 'W') ret.first--;
		else if (prog[idx] == 'E') ret.first++;
		else if (prog[idx] == ')') {
			break;
		}
		else {
			...
			idx += 2;
			pll move = eval();
			ret.first += (move.first * mult) % MOD;
			ret.second += (move.second * mult) % MOD;
		}
//		cout << idx << ' ' << prog[idx] << " " << to_string(ret) << endl;
	}
	while (ret.first < 0) ret.first += MOD;
	while (ret.second < 0) ret.second += MOD;
...
void solve(int tc) {
	...
	cout << "Case #" << tc << ": " << pos.first << " " << pos.second << endl;
}
