//W1C.cpp created at 10/29/21 14:50:28
...
void dijkstra(const int s) {
    ...
    while (pq.size() > 0) {
        ...
        if (cur.fi > G[s][cur.se]) continue;

        for (const pcll& p: adj[cur.se]) {
            if (chmn(G[s][p.fi], cur.fi + p.se)) {
                pq.emplace(G[s][p.fi], p.fi);
            }
        }
    }
}
...
void solve(int testcase) {
    ...
    for (int i=0; i<M; ++i) {
        ...
        cin >> B[i].fi >> B[i].se >> w;
        --B[i].fi; --B[i].se;
        adj[B[i].fi][B[i].se] = adj[B[i].se][B[i].fi] = w;
    }
    ...
    for (int i=0; i<M; ++i) {
        ...
        for (int j=0; j<K; ++j) {
            cur += min({
                G[A[j].fi][B[i].fi] + G[A[j].se][B[i].se],
                G[A[j].se][B[i].fi] + G[A[j].fi][B[i].se],
                G[A[j].fi][A[j].se]
            });
        }
        chmn(ans, cur);
