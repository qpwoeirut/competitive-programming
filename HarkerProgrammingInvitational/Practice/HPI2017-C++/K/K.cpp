...
struct Item {
	...
	Item(int _r, int _c, int _dir, int _dist) {
		r = _r;
		c = _c;
		dir = _dir;
		dist = _dist;
	...
	bool valid() {
		return 0 <= r && r < R && 0 <= c && c < C && grid[r][c] != '*';
	}
};
...
int main() {
	...
	while (q.size() > 0) {
		...
		if (grid[cur.r][cur.c] == 'C' && (cur.r != sr || cur.c != sc)) {
			cout << cur.dist << endl;
			return 0;
		}

		for (int i=0; i<4; i++) {
			Item nxt(cur.r + chr[i], cur.c + chc[i], i, cur.dist + (i != cur.dir));
			if (v[nxt.r][nxt.c][nxt.dir] || !nxt.valid()) continue;
			v[nxt.r][nxt.c][nxt.dir] = true;
