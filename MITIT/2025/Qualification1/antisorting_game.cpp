//antisorting_game.cpp created at 04/06/25 09:28:13
#include <bits/stdc++.h>
...
void solve(int testcase) {
    ...
    int lo = 0, hi = N;
    for (; lo < N && S[lo] == '0'; ++lo);
    for (; hi > 0 && S[hi-1] == '1'; --hi);

    int n = hi - lo;
    bool bad = is_sorted(S.begin(), S.end()) || n <= 2;
    for (int i=0; i+i<n; ++i) {
        bad |= S[lo+i] != '1';
        bad |= S[hi-i-1] != '0';
    }
    ...
    while (true) {
        if (S == "-1") exit(0);
        if (is_sorted(S.begin(), S.end())) return;

        lo = 0, hi = N;
        for (; lo < N && S[lo] == '0'; ++lo);
        for (; hi > 0 && S[hi-1] == '1'; --hi);

        const int zeros = count(S.begin() + lo, S.begin() + hi, '0');
        const int ones = count(S.begin() + lo, S.begin() + hi, '1');
        if (zeros == 1) {
            cout << hi - lo - 1 << '\n';
