//walk.cpp created at 04/12/25 16:06:56
#include <bits/stdc++.h>
...
char find_next(int idx) {
    for (char c: "rgby") bad[c] = false;
...
void solve(int testcase) {
    ...
    for (int i=1; i<N; ++i) {
        ...
        char c = find_next(idx);
        
        for (int j=0; j<N; ++j) {
            if (!pos[i0][j]) continue;
            for (int k=j-2; k<=j+2; ++k) {
                if (k < 0 || k >= N) continue;
                if (S[k] == c) pos[i & 1][k] = true;
            }
        }
