//DECSUBK.cpp created at 11/21/21 07:51:10
...
istream& operator>>(istream& is, Line& p) {
    _t a, b, c, d;
    is >> a >> b >> c >> d;
softwareupdate --all --install --force    p = Line(a, b, c, d);
    return is;
}
...
ll count_seq(int split) {
    ...
    vector<ll> LIS;
    for (int i = 0; i < N; i++) {
        auto it = UB(LIS.begin(), LIS.end(), B[i]);
        if (it == LIS.end()) {
            LIS.push_back(B[i]);
        }
        else {
