...
void solve() {
	...
	Pipe pipes[MN];
	for (int i=0; i<N; i++) {
		cin >> pipes[i].down >> pipes[i].x >> pipes[i].y;
	}
	...
	for (int i=0; i<N; i++) {
		...
		double top = H, bot = -H;
		for (int j=i-1; j>=0; j--) {
			double x_diff = pipes[i].x - pipes[j].x;
			if (pipes[j].down) {
				if (pipes[j].y < pipes[i].y + (top * x_diff)) {
					top = double(pipes[j].y - pipes[i].y) / x_diff;
					if (top <= bot) break;
					ans[i]++;
				}
			}
			else {
				if (pipes[j].y > pipes[i].y + (bot * x_diff)) {
					bot = double(pipes[j].y - pipes[i].y) / x_diff;
					if (top <= bot) break;
					ans[i]++;
				}
			}
		...
		top = H, bot = -H;
		for (int j=i+1; j<N; j++) {
			double x_diff = pipes[j].x - pipes[i].x;
			if (pipes[j].down) {
				if (pipes[j].y < pipes[i].y + (top * x_diff)) {
					top = double(pipes[j].y - pipes[i].y) / x_diff;
					if (top <= bot) break;
