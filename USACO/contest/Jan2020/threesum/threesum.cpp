...
int main() {
	...
	for (int i=0; i<N; i++) {
		for (int j=i+1; j<N; j++) {
			map<int, vector<int> >::iterator it=pos.find(-(A[i] + A[j]));
			if (it == pos.end()) continue;
			ll triplets = upper_bound(it->second.begin(), it->second.end(), j-1) - lower_bound(it->second.begin(), it->second.end(), i+1);
			for (int q=0; q<Q; q++) {
				if (qu[q].L <= i && j <= qu[q].R) {
					qu[q].ans += triplets;
				}
			}
		}
