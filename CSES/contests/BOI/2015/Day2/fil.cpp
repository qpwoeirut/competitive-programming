...
bool solve(int node) {
    ...
    while (node != -1) {
        ...
        for (const int d: divs[x]) {
            ...
            //if (pos[t].size() <= 8) {
            //    for (const int p: pos[t]) {
            //        if (mark[node].fi <= p && p <= mark[node].se) return true;
            //    }
            //} else 
            if (LB(all(pos[t]), mark[node].fi) != UB(all(pos[t]), mark[node].se)) return true;
        }
