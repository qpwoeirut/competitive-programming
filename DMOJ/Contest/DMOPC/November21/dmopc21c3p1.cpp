//dmopc21c3p1.cpp created at 11/13/21 12:19:02
...
ll ask(const int i, const int j) {
    cout << "? " << i+1 << ' ' << j+1 << endl;
    ll x;
    cin >> x;
    return x;
...
void solve(int testcase) {
    ...
    for (int i=0; i+1<N; i+=2) {
        const ll a_minus_b = ask(i, i+1);
        const ll b = ask(i, i+1) / 2;
        const ll a = a_minus_b + b;
