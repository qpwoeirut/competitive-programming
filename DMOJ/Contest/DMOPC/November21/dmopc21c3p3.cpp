//dmopc21c3p3.cpp created at 11/13/21 12:19:05
...
int hop(int i) {
    if (A[i] == 0) return 0;
    if (0 <= i+A[i] && i+A[i] <= N+M && A[i+A[i]] == 0) {
        swap(A[i], A[i+A[i]]);
        hops.pb(i);
        debug1(A, N+M+1);
        return 1;
    }
    if (0 <= i+2*A[i] && i+2*A[i] <= N+M && A[i+A[i]] == -A[i] && A[i+(2*A[i])] == 0) {
        swap(A[i], A[i+(2*A[i])]);
        hops.pb(i);
        debug1(A, N+M+1);
        return 2;
...
void solve(int testcase) {
    ...
    while (p1+1 <= N+M || p2 > 0) {
        const int t1 = p1 - 2;
        p1 += hop(p1);
        for (int i=t1; i>=0 && i>=p2-2; --i) {
            if (A[i] == 1) hop(i);
        }
        const int t2 = p2 + 2;
        p2 -= hop(p2);
        for (int i=t2; i<=N+M && i<=p1+2; ++i) {
            if (A[i] == -1) hop(i);
        }
    }
