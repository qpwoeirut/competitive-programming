//371820i.cpp created at 03/05/22 13:26:59
...
void solve(int testcase) {
    ...
    for (int i=1; i<N; ++i) {
        for (int j=1; j<MX; ++j) {
            dp[i % MX][j] = INF;
            if (A[i] >= j && pos[A[i] - j] != -1) {
                if (j > 1) chmn(dp[i % MX][j], dp[pos[A[i] - j]][j - 2] + 1);
                chmn(dp[i % MX][j], dp[pos[A[i] - j]][j - 1] + 1);
                chmn(dp[i % MX][j], dp[pos[A[i] - j]][j + 1] + 1);
                chmn(dp[i % MX][j], dp[pos[A[i] - j]][j + 2] + 1);
            }
        }
        dp[i % MX][0] = min(INF, min(dp[i % MX][1], dp[i % MX][2]) + 1);
    }
    ll ans = INF;
    for (int j=0; j<MX; ++j) {
        chmn(ans, dp[(N-1) % MX][j]);
    }
    cout << (ans == INF ? -1 : ans) << '\n';
}
