//371820g.cpp created at 03/05/22 13:11:05
...
struct Item {
    int id, pts, cost, deadline;
    const inline bool operator<(const Item& o) const {
        return id < o.id;
    }
};
...
void solve(int testcase) {
    cin >> N >> K;
    for (int i=1; i<=N; ++i) {
        cin >> A[i].id >> A[i].pts >> A[i].cost >> A[i].deadline;
    }
    sort(A+1, A+N+1);
    ...
    for (int i=1; i<=N; ++i) {
        for (int j=1; j<=K; ++j) {
            ...
            if (j >= A[i].cost && j <= A[i].deadline) {
                chmx(dp[i][j], dp[i-1][j-A[i].cost] + A[i].pts);
            }
        }
    }
