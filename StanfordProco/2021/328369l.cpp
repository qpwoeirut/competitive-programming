//328369l.cpp created at 05/15/21 14:37:58
...
bool V[MN];
int dfs(const int u, const int idx) {
    debug(u, idx);
    if (V[u]) return 0;
    V[u] = true;
    int ret = 1;
...
int brute() {
    ...
    for (int i=0; i<n; ++i) {
        // i is counted twice
        fill(V, V+n, false);
        const int ct1 = dfs(i, 0);
        fill(V, V+n, false);
        const int ct2 = dfs(i, 1);
...
void test() {
    for (int t=0; t<10000; ++t) {
        for (n=2; n<=6; ++n) {
            for (m=n-1; m<=20; ++m) {
                fill(adj[0], adj[0]+n, set<ll>());
                fill(adj[1], adj[1]+n, set<ll>());
                edges.clear();
                redges.clear();
                for (int i=0; i<m; ++i) {
