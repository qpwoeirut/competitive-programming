//438970k.cpp created at 04/15/23 10:43:49
...
int ssolve(pt a, pt b, pt c) {
    a -= b;
    c -= b;
    long double a1 = arg(a);
    long double a2 = arg(c);
    long double ang = abs(a1 - a2) / pi;
    // cancel 180

    fill(A, A+401, 2);
    for (int n=3; n<=400; ++n) {
        long double val = ang * n;
        debug(n, val);
        val -= (int)(val);
        A[n] = min(val, 1.0 - val);
    }
    return min_element(A, A+401) - A;
}
