...
int EARLY_FACTOR = 15;
#else
const int EARLY_PLANT = 5;
const int EARLY_FACTOR = 15;
#endif
...
void init() {
    ...
    for (int t = 0; t <= T; ++t) {
        for (int t0 = 1; t0 <= EARLY_PLANT && t + t0 <= T; ++t0) {
            plant_list[t].insert(plant_list[t].end(), plant_list[t + t0].begin(), plant_list[t + t0].end());
        }
        sort(plant_list[t].begin(), plant_list[t].end(), [](const int ia, const int ib) {
            return crop[ia].harvest - EARLY_FACTOR * crop[ia].plant > crop[ib].harvest - EARLY_FACTOR * crop[ib].plant;
        });
    }
...
int n_comp;
int cur_comp;
void dfs_component(const int r, const int c) {
    if (visited[r][c] || in_ngraph[r][c]) return;
    visited[r][c] = true;

    comp[cur_comp++] = pii(r, c);
    for (int dir = 0; dir < 4; ++dir) {
        if (wall[r][c] & (1 << dir)) continue;
        dfs_component(r + chr[dir], c + chc[dir]);
    }
}
void find_components() {
    ...
    for (int r = 0; r < H; ++r) {
        for (int c = 0; c < W; ++c) {
            if (!visited[r][c] && !in_ngraph[r][c]) {
                dfs_component(r, c);
                comp_idx[n_comp++] = cur_comp;
            }
        }
    }
}
...
pii comp_val[H * W];  // range [lo, hi)
void set_plant_location(const int t, const int k) {
    work[k] = Work(k, -1, -1, t);
    int min_diff = INF;
    ...
    if (work[k].i == -1) {
        int min_change = INF + 1;
        for (int i = 0; i < n_comp; ++i) {
            const int change = comp_val[i].fi == INF ? INF : 
                (crop[k].harvest < comp_val[i].fi ? comp_val[i].fi - crop[k].harvest :
                    (comp_val[i].se <= crop[k].harvest ? (crop[k].harvest - comp_val[i].se + 1) : 0));
            if (comp_val[i].se - comp_val[i].fi + change <= COMP_RANGE || min_change == INF + 1) {
                for (int j = comp_idx[i - 1]; j < comp_idx[i]; ++j) {
                    if (min_change > change && user[comp[j].row][comp[j].col] == -1) {
                        min_change = change;
                        comp_val[i] = comp_val[i].fi == INF ? pii(crop[k].harvest, crop[k].harvest) : 
                            pii(min(comp_val[i].fi, crop[k].harvest), max(crop[k].harvest + 1, comp_val[i].se));
                        work[k].i = comp[j].row;
                        work[k].j = comp[j].col;
                        break;
                    }
...
int create_plan() {
    ...
    for (int t = 1; t <= T; ++t) {
        for (int i = 0; i < n_comp; ++i) {
            ...
            if (comp_val[i].fi >= comp_val[i].se) {
                comp_val[i] = pii(INF, INF + T);
            }
        }
        // planting phase
        for (const int k : plant_list[t]) {
            if (!in_plan[k]) set_plant_location(t, k);
        }
