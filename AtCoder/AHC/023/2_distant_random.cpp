...
Work work[MAX_K];
vector<Work> plant_list[T + 1];
vector<Work> harvest_list[T + 1];

geometric_distribution<int> distrib(0.018);
int create_random_plan() {
    ...
    for (int k = 0; k < K; ++k) {
        const int pos_idx = max(9, H * W - distrib(rng) - 1);
        work[k] = Work(k, position[pos_idx].row, position[pos_idx].col, crop[k].plant);
