//abc312g.cpp created at 07/28/23 22:40:02
...
ll dfs(int u, int par) {
    ...
    for (int v: adj[u]) {
        ...
        ll other = ssize[u] - ssize[v];
        child_paths += dp[v] * other;
        root_paths += ssize[v] * (other - 1);
        debug(u, v, dp[v], ssize[v], other);
    }
    assert(root_paths % 2 == 0);
    root_paths /= 2;

    ll ans = old_paths + child_paths + root_paths;
    debug(u, old_paths, child_paths, root_paths);
    return ans;
}
