//abc312b.cpp created at 07/28/23 22:39:52
...
bool tak(int r, int c) {
    if (r+9 > N || c+9 > M) return false;
    for (int i=0; i<4; ++i) {
        for (int j=0; j<4; ++j) {
            bool white = (i == 3) || (j == 3);
            bool gwhite = G[r+i][c+j] == '.';
            if (white != gwhite) return false;

            gwhite = G[r+9-i-1][c+9-j-1] == '.';
            if (white != gwhite) return false;
        }
    }
    return true;
