...
int t_idx[1 << 27];
long long t_val[1 << 27];

int available = 0;

int get_table(const int board, const int depth) {
    if (t_idx[board] == 0) return -1;

    for (int i = t_idx[board] - 1; (i & 0xF) > 0; --i) {
        if ((t_val[i] & 0xFF) == depth) return t_val[i] >> 8;
    }
    return -1;
}

int set_table(const int board, const int depth, const int value) {
    if (t_idx[board] == 0) {
        t_idx[board] = available;
        available += 16;
    }
    t_val[t_idx[board]++] = ((long long)(value) << 8) | depth;
    return value;
}
...
uint32_t solve(const int board, const int depth) {
    ...
    if (depth == 0 || (~(((board & 0333333333) + 0333333333) | board) & 0444444444) == 0) {
        return
            ((board >> 24)    ) * 100000000 +
            ((board >> 21) & 7) * 10000000 +
            ((board >> 18) & 7) * 1000000 +
    ...
    const int cache = get_table(board, depth);
    if (cache != -1) return cache;

    return set_table(board, depth, (
