//weak_typing1.cpp created at 09/11/21 22:04:08
...
void setIO(const string& filename = "weak_typing1") {
    if (filename.size() > 0) {
...
void solve(int testcase) {
    ...
    ll ans = 1;
    bool ox = o_idx[0] < x_idx[0];
    int i=0, j=0;
    while (i < sz(o_idx) && j < sz(x_idx)) {
        bool new_ox = o_idx[i] < x_idx[j];

        if (ox != new_ox) {
            ++ans;
            ox = new_ox;
        }
        if (o_idx[i] < x_idx[j]) ++i;
        else if (o_idx[i] > x_idx[j]) ++j;
    }
