...
void solve(int tc) {
	...
	sort(A, A+N, cmpf<int,int>());

	ll ans = 0;
	mll len;
	for (int i=0; i<N; i++) {
		mll::iterator it=len.find(A[i].FI);
		if (it != len.end()) {
			chmx(len[A[i].FI + A[i].SE], A[i].SE + it->SE);
		}
		else {
			chmx(len[A[i].FI + A[i].SE], A[i].SE);
		}


		it=len.find(A[i].FI - A[i].SE);
		if (it != len.end()) {
			chmx(len[A[i].FI], A[i].SE + it->SE);
		}
		else {
			chmx(len[A[i].FI], A[i].SE);
		}
	}

	for (mll::iterator it=len.begin(); it!=len.end(); it++) {
		chmx(ans, it->SE);
	}
