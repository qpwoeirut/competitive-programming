//ready_go.cpp created at 10/21/23 10:01:30
...
void setIO(const string& filename = "ready_go") {
    if (filename.size() > 0) {
...
int R, C;
int spaces, empty_row, empty_col;
int dfs(int r, int c, int fill_id) {
    if (r < 0 || r >= R || c < 0 || c >= C) return 0;
    if (vis[r][c] >= fill_id) return 0;
    if (A[r][c] == 'B') return 0;
...
void solve(int testcase) {
    ...
    for (int r=0; r<R; ++r) {
        for (int c=0; c<C; ++c) {
            if (vis[r][c] == -1 && A[r][c] == 'W') {
                spaces = 0;
                empty_row = R;
                empty_col = C;
                int group_size = dfs(r, c, r * C + c);
                if (spaces == 1) {
                    G[empty_row][empty_col] += group_size;
                }
            }
        }
    }
    ...
    //cout << "Case #" << testcase << ": " << (ans > 0 ? "YES" : "NO") << '\n';
