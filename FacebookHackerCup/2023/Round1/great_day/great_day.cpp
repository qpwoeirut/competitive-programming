//great_day.cpp created at 10/07/23 11:28:46
...
void setIO(const string& filename = "great_day") {
    if (filename.size() > 0) {
...
struct Node {
	Node *l = 0, *r = 0;
	int lo, hi;
    pll val = pll(-inf, -inf), oval = pll(-inf, -inf);
    bool mflip = false;
	...
	Node(vector<ll>& v, int lo, int hi) : lo(lo), hi(hi) {
		if (lo + 1 < hi) {
			...
			val = max(l->val, r->val);
            oval = max(l->oval, r->oval);
		}
		else {
            val = pll(v[lo], -lo);
            oval = pll(MOD - v[lo], -lo);
        }
	}
	pll query(int L, int R) {
		if (R <= lo || hi <= L) return pll(-inf, -inf);
