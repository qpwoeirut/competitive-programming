//sum_41.cpp created at 10/07/23 10:20:47
...
void setIO(const string& filename = "sum_41") {
    if (filename.size() > 0) {
...
void solve(int testcase) {
    ...
    q.emplace(0, 0);
    while (q.size() > 0) {
        ll sum = q.front().fi, prod = q.front().se; q.pop();
        if (sum == 41 && prod == K) {
            vector<ll> ans;
            while (sum > 0) {
                const ll d = last[sum][prod];
                assert(d > 0);
                assert(sum >= d);
                assert(prod % d == 0);
        ...
        for (ll d: divs) {
            ll nsum = sum + d;
            ll nprod = prod == 0 ? d : prod * d;
            if (nsum <= 41 && K % nprod == 0 && last[nsum].count(nprod) == 0) {
                q.emplace(nsum, nprod);
                last[nsum][nprod] = d;
            }
        }
    }
