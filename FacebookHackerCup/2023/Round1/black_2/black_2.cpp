//black_2.cpp created at 10/07/23 12:12:36
...
void setIO(const string& filename = "black_2") {
    if (filename.size() > 0) {
...
void solve(int testcase) {
    ...
    for (int i=1; i<=N; ++i) {
        if (state[i]) {
            to_press.ins(i);
            for (int j=i; j<=N; j+=i) state[j].flip();
        }
    }
    ...
    for (int q=0; q<Q; ++q) {
        int p;
        cin >> p;
        pair<set<ll>::iterator, bool> res = to_press.ins(p);
        if (res.se == false) {
            to_press.erase(res.fi);
        }
