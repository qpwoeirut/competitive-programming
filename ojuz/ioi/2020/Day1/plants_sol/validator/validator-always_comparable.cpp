...
void Do() {
	...
	while (Q--) {
		...
		inf.readEoln();
		ensuref(Query(a+1, b+1)!=0, "plants are not comparable!");
	}
}
...
int main() {
	...
	n = inf.readInt(MINN, MAXN, "n");
	inf.readSpace();
	K = inf.readInt(MINK, MAXK, "k");
	inf.readSpace();
	Q = inf.readInt(MINQ, MAXQ, "q");
	inf.readEoln();
	for (i = 1; i <= n; i++) {
        if(i!=1) inf.readSpace();
		Rank[i] = inf.readInt(0, K-1, "r_i")+1;
	}
	inf.readEoln();
	IT.init(1, 1, n);
	for (i = 1; i <= n; i++) {
