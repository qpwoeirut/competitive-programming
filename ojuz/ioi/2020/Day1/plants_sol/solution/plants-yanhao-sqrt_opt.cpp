...
void init(int k, std::vector<int> r) {
    ...
    if(n>5000 && k<=n/2) {
        ...
        sqrt_rg[n] = n;
        for(int i=0; i<n; i++) {
            while(lt_dist(sqrt_lt[i],i)<2000 && sqrt_lt[i]!=n) {
                sqrt_lt[i] = lt[sqrt_lt[i]];
            }
            while(lt_dist(i,sqrt_rg[i])<2000 && sqrt_rg[i]!=n) {
...
bool lt_path(int x, int y) {
    if(n>5000 && k_global<=n/2) {
        while(lt_dist(x,y)>=2000+k_global && sqrt_lt[y]!=n) {
            y = sqrt_lt[y];
            if(permutation[y]>permutation[x]) return false;
        }
    }
...
bool rg_path(int x, int y) {
    if(n>5000 && k_global<=n/2) {
        while(lt_dist(y,x)>=2000+k_global && sqrt_rg[y]!=n) {
            y = sqrt_rg[y];
            if(permutation[y]>permutation[x]) return false;
        }
    }
...
int compare_plants(int x, int y) {
    if(permutation[x]>permutation[y] && path(x,y)) return -1;
    if(permutation[y]>permutation[x] && path(y,x)) return 1;
    return 0;
}
