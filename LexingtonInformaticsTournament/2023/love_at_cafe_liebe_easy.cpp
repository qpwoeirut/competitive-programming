//love_at_cafe_liebe_easy.cpp created at 08/05/23 16:42:12
...
void solve(int testcase) {
    ...
    cin >> S;
    for (int i=0; i<M; ++i) {
        cin >> A[i].a >> A[i].x >> A[i].b >> A[i].y >> A[i].c >> A[i].z;
        --A[i].x; --A[i].y; --A[i].z;
    }
    ...
    while (pq.size() > 0) {
        ...
        for (int i=0; i<M; ++i) {
            if (A[i].x == cur.se || A[i].y == cur.se) {
                if (chmn(cost[A[i].z], A[i].a * cost[A[i].x] + A[i].b * cost[A[i].y])) {
                    pq.emplace(cost[A[i].z], A[i].z);
                }
            }
        }
    }
