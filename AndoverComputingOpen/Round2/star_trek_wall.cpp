//star_trek_wall.cpp created at 05/21/22 14:57:05
...
void solve(int testcase) {
    ...
    for (int i=0; i<N; ++i) {
        ...
        ++corner[A[i].fi][A[i].se];
        --corner[A[i].fi][B[i].se + 1];
        --corner[B[i].fi + 1][A[i].se];
        ++corner[B[i].fi + 1][B[i].se + 1];
    }
    ...
    for (int i=0; i<N; ++i) {
        ...
        int lo = MN, hi = 0, lef = MN, ri = 0;
        bool covered = true;
        for (int r=1; r<=20; ++r) {
            for (int c=1; c<=20; ++c) {
                cover[r][c] = corner[r][c] - cover[r-1][c-1] + cover[r-1][c] + cover[r][c-1];
                if (cover[r][c] == 0) {
                    covered = false;
                    chmn(lo, r);
                    chmn(lef, c);
        ...
        if (dr <= B[i].fi - A[i].fi + 1 && dc <= B[i].se - A[i].se + 1) {
            cout << "Yes\n";
            return;
        }
        if (covered) {
