//hopscotch.cpp created at 05/21/22 14:28:52
...
void solve(int testcase) {
    ...
    for (int i=0; i<N; ++i) {
        for (int j=0; j<N; ++j) {
            ...
            for (int ii=max(0LL, j - K); ii<i; ++ii) {
                dp[i][j] = max(dp[i][j], dp[ii][j] + A[i]);
            }
            for (int jj=max(0LL, i - K); jj<j; ++jj) {
                dp[i][j] = max(dp[i][j], dp[i][jj] + B[j]);
            }
        }
    }
