//444573c.cpp created at 05/20/23 13:38:46
...
void solve(int testcase) {
    ...
    for (int i=0; i<N+M; ++i) {
        if (K == 0 || B[K].fi != A[i].fi) B[K++] = A[i];
        else chmx(B[K-1].se, A[i].se);
    }

    ll cur = 0;
    for (int i=0; i<K; ++i) {
        if (i) {
            cur = max(0ll, cur - (A[i].fi - A[i-1].fi));
        }
        chmx(cur, A[i].se);
        chmx(light[i], cur);
    }
    ...
    for (int i=K-1; i>=0; --i) {
        if (i+1 < K) {
            cur = max(0ll, cur - (A[i+1].fi - A[i].fi));
        }
        chmx(cur, A[i].se);
        chmx(light[i], cur);
    }
    ...
    for (int i=0; i<M; ++i) {
        int idx = lower_bound(B, B+K, pll(query[i], 0)) - B;
        cout << light[idx] << '\n';
    }
    debug1(B, K);
    debug1(light, K);
