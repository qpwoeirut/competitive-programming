//cyclic_shifts.cpp created at 04/02/23 09:47:51
...
void solve(int testcase) {
    ...
    ll ans = psum[N];
    for (int i=0; i+K<=N; ++i) {
        ll shifted = spsum[i+K] - spsum[i+1] + abs(A[i] - B[i+K-1]);
        ll normal = psum[N] - psum[i+K] + psum[i];
        debug(i, shifted, normal);
        chmn(ans, shifted + normal + 1);
    }
    cout << ans << '\n';
}
