//trivial_problem.cpp created at 08/16/25 12:18:16
#include <bits/stdc++.h>
...
void solve(int testcase) {
    ...
    for (int v=0; v<N; ++v) {
        B[v] = 0;
        for (int i: idx[v]) {
            if (i && A[i-1] <= A[i]) join(i-1, i);
            if (i+1<N && A[i+1] <= A[i]) join(i+1, i);
        }

        for (int i: idx[v]) {
            debug(v, i, root(i), sz[root(i)]);
            chmx(B[v], sz[root(i)]);
        }
        if (v) chmn(B[v], B[v-1]);
    }
    ...
    for (int x=1; x<=N; ++x) {
        int n = upper_bound(B, B+N+1, x, greater<ll>()) - B;
        cout << n << ' ';
    }
    cout << '\n';
}
