//castlefall.cpp created at 08/16/25 13:56:57
#include <bits/stdc++.h>
...
bitset<MN> A[20];
pii ans;
void recurse(int i, const bitset<MN>& a, const bitset<MN>& b) {
    if (i == N) {
        if (a.count() == 0 || b.count() == 0 || b[M]) return;
        ...
        bitset<MN> a0(a), b0(b);
        if (b.count() == 1) a0[cur.se] = false; 
        else if (a.count() == 1) b0[cur.fi] = false;

        if (a0.count() > 1 || b0.count() > 1) {
            ans = {M, M};
            return;
        }
        debug(a0[0]+0, a0[1]+0, b0[0]+0, b0[1]+0);
        ...
        if (cur.fi != cur.se) {
            if (ans.fi == -1) ans = {M-cur.fi-1, M-cur.se-1};
            else ans = {M, M};
        }
        return;
    }
...
void solve(int testcase) {
    ...
    bitset<MN> all;
    for (int i=0; i<=M; ++i) all[i] = true;

    ans = {-1, -1};
    recurse(1, A[0], all);
    if (ans.fi != -1 && ans.fi != M) {
