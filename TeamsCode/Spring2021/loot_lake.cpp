//loot_lake.cpp created at 04/24/21 13:34:36
...
void solve(int testcase) {
    ...
    while (q.size() > 0) {
        ...
        for (int i=0; i<4; ++i) {
            const ll nr = p.fi + chr[i];
            const ll nc = p.se + chc[i];
            if (nr <= 0 || nr > N || nc <= 0 || nc > M || G[nr][nc]) continue;
            if (chmn(dist[nr][nc], dist[p.fi][p.se] + 1)) {
                q.emplace(nr, nc);
            }
            if (dist[nr][nc] == dist[p.fi][p.se] + 1) {
                ct[nr][nc] += ct[p.fi][p.se];
                if (ct[nr][nc] >= MOD) ct[nr][nc] -= MOD;
            }
        }
    }
