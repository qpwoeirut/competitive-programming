//ciphered_statistics.cpp created at 04/23/21 17:36:34
...
void solve(int testcase) {
    ...
    ll sum = 0;
    for (int i=0; i<N; ++i) {
        ll median = lo.empty() ? 0 : (sz(lo) > sz(hi) ? *lo.begin() : (*lo.begin() + *hi.begin()) / 2);
        A[i] ^= median;
        ...
        if (A[i] <= *lo.begin()) lo.ins(A[i]);
        else hi.ins(A[i]);

        if (sz(lo) < sz(hi)) {
            lo.insert(*hi.begin());
            hi.erase(hi.begin());
        } else if (sz(lo) > sz(hi) + 1) {
            hi.insert(*lo.begin());
            lo.erase(lo.begin());
        }

        median = lo.empty() ? 0 : (sz(lo) > sz(hi) ? *lo.begin() : (*lo.begin() + *hi.begin()) / 2);

        assert(!lo.empty());
        ll mn = *lo.rbegin();
        ll mx = hi.empty() ? mn : *hi.rbegin();
        cout << mn << ' ' << mx << ' ' << sum << ' ' << median << ' ' << mode.se << '\n';
    }
}
