//not_japanese_triangle.cpp created at 04/05/25 13:17:22
#include <bits/stdc++.h>
...
void solve(RMQ<pll>& rmq, int a, int b) {
    if (a == b) return;
    pll min_el = rmq.query(a, b);

    int lo = N - (b - a);
    int x = min(min_el.se - a + 1, b - min_el.se);
    for (int i=0; i<x; ++i) {
        B[lo + i] += min_el.fi;
        B[N - i] -= min_el.fi;
    }

    solve(rmq, a, min_el.se);
    solve(rmq, min_el.se + 1, b);
}
