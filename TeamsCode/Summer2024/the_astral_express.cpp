//the_astral_express.cpp created at 07/28/24 14:22:55
...
set<int> vis[MN];
ll trip(const vector<int>& arr, int vel) {
    for (int i=0; i<2*N; ++i) vis[i].clear();
    ...
    while (0 <= idx && idx < 2*N) {
        ...
        if (vel > 0) last = 1;
        else if (vel < 0) last = -1;
        idx += last;

        if (idx < 0 || idx >= arr.size()) break;

        vel += arr[idx];
        if (vis[idx].count(vel)) return -1;
        vis[idx].ins(vel);
    }
    return ct;
}
