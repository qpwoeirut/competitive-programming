//2124c.cpp created at 07/06/25 07:51:12
#include <bits/stdc++.h>
...
bool check(ll x) {
    bool ok[2] = {true, B[0] % x == 0};
    for (int i=1; i<N; ++i) {
        bool ok0 = (ok[0] && (B[i] % B[i-1] == 0)) || (ok[1] && (B[i] % (B[i-1] / x) == 0));
        bool ok1 = (B[i] % x == 0) && ((ok[0] && (B[i] / x) % B[i-1] == 0) || (ok[1] && (B[i] / x) % (B[i-1] / x) == 0));
        ok[0] = ok0;
        ok[1] = ok1;
        //debug(x, i, ok[0], ok[1]);
    }
