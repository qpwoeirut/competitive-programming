//2124e.cpp created at 07/06/25 09:34:35
#include <bits/stdc++.h>
...
int solve() {
    ...
    ll extra = B[N], b_i = 0;
    for (int i=0; i<=N; ++i) {
        if (chmn(extra, abs(B[N] - 2 * B[i]))) b_i = i;
    }
    ans[0] = vector<ll>(A, A+N);
    if (B[b_i] * 2 == B[N]) {
        return 1;
    }

    if (B[b_i] * 2 < B[N]) {
        for (int i=0; i<b_i; ++i) arr[i] = 0;
    } else {
        for (int i=b_i; i<N; ++i) arr[i] = 0;
    }

    ans[1] = vector<ll>(N);
    ll cur = extra / 2;
    for (int j=b_i; j<N; ++j) {
        ans[1][j] = min(ans[0][j], cur);
        ans[0][j] -= ans[1][j];
        cur -= ans[1][j];
        if (cur == 0) break;
    }
    assert(cur == 0);
    cur = extra / 2;
    for (int j=N-1; j>=b_i; --j) {
        ans[1][j] = min(ans[0][j], cur);
        ans[0][j] -= ans[1][j];
        cur -= ans[1][j];
        if (cur == 0) break;
    }
    debug(extra, cur, ans[0], ans[1]);

    for (int i=0; i<N; ++i) {
        assert(A[i] == ans[0][i] + ans[1][i]);
    }
    return 2;
}
