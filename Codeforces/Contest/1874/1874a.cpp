//1874a.cpp created at 09/30/23 07:36:26
...
void solve(int testcase) {
    ...
    multiset<ll> a(A, A+N), b(B, B+M);
    for (int i = 1; i <= K && i <= (K % 2) + 20; ++i) {
        ll from_a = (i & 1) ? *a.begin() : *a.rbegin();
        ll from_b = (i & 1) ? *b.rbegin() : *b.begin();
        if ((from_a < from_b && (i & 1)) || (from_b < from_a && ((i & 1) == 0))) {
            a.ins(from_b);
            b.erase(b.find(from_b));

            a.erase(a.find(from_a));
            b.ins(from_a);
        }
    }
