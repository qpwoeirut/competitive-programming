//1870e.cpp created at 09/18/23 09:34:32
...
void solve(int testcase) {
    ...
    fill(first_seen, first_seen+N+1, N);
    fill(last_used, last_used+N+1, -1);

    for (int i=0; i<N; ++i) {
        chmn(first_seen[mex[i][i]], i);

        for (int j=i; j>=0; --j) {
            if (last_used[mex[j][i]] >= j) continue;
            last_used[mex[j][i]] = j;

            for (int v=0; v<N; ++v) {
                if (first_seen[v] < N) chmn(first_seen[v ^ mex[j][i]], i);
            }
        }
    }
