//1870d.cpp created at 09/18/23 07:56:03
...
void solve(int testcase) {
    ...
    for (int i=0; i<N; ++i) {
        ...
        debug(i, idx, A[idx], B[idx], rem, last_idx);
        if (last_idx != -1) {
            ll diff = A[idx] - A[last_idx];
            ll move = diff == 0 ? B[last_idx] : min(rem / diff, B[last_idx]);
            B[last_idx] -= move;
            B[idx] += move;
            rem -= move * diff;
            debug(diff, move, rem);
