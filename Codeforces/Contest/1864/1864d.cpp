//1864d.cpp created at 08/26/23 08:01:14
...
void solve(int testcase) {
    ...
    for (int r=2; r<=N+1; ++r) {
        for (int c=1; c+1<M; ++c) {
            incsum[r][c] = incsum[r-1][c-1] ^ incsum[r-1][c+1] ^ incsum[r-2][c] ^ inc[r-1][c];
            G[r][c] = G[r][c] ^ incsum[r][c];

            if (G[r][c] > 0 && K <= c && c < N+K) {
                G[r][c].flip();
                inc[r][c] = 1;
                incsum[r][c].flip();
                ++ans;
