//1991f.cpp created at 07/28/24 10:21:13
...
// this is incorrect
bool ok(const multiset<int>& nums) {
    int arr[6] = {-1, -1, -1, -1, -1, -1};
    for (int x: nums) {
        if (arr[5] != -1) {
            for (int i=1; i<6; ++i) arr[i-1] = arr[i];
            arr[5] = x;
        } else {
            for (int i=0; i<6; ++i) {
                if (arr[i] == -1) {
        ...
        debug1(arr, 6);
        do {
            if (check(arr[0], arr[1], arr[2]) && check(arr[3], arr[4], arr[5])) {
                return true;
            }
