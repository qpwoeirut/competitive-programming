//1991d.cpp created at 07/28/24 08:12:52
...
void solve(int testcase) {
    ...
    if (N <= 5) {
        ...
        A[0] = -1;
        for (int u=1; u<=N; ++u) A[u] = (u % 4) + 1;
        //for (int u=1; u<=N; ++u) {
        //    for (int p=0; p<n_prime; ++p) {
        //        const int v = u ^ primes[p];
        //        if (A[u] == A[v]) debug(u, v);
        //    }
        //}
    }
...
int main() {
    ...
    for (int n=2; n<MN; ++n) {
        if (!prime[n]) continue;
        primes[n_prime++] = n;
        for (int i=2*n; i<MN; i+=n) prime[i] = false;
    }
