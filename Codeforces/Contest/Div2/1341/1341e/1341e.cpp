...
bool seen[MN][1001];
map<int, set<int> > dp[MN];
set<int> dfs(int idx, int rem) {
	if (rem < 0) {
		return set<int>();
	}
	if (seen[idx][rem]) {
	...
	if (rem == 0) {
		dp[idx][rem] = set<int>();
		dp[idx][rem].insert(idx);
		return dp[idx][rem];
	}

	if (idx > 0) {
		set<int> tmp = dfs(idx-1, rem - (A[idx] - A[idx-1]));
		dp[idx][rem].insert(tmp.begin(), tmp.end());
	}
	if (idx+1 < M) {
		set<int> tmp = dfs(idx+1, rem - (A[idx+1] - A[idx]));
		dp[idx][rem].insert(tmp.begin(), tmp.end());
	}
...
//	debug(adj);
	...
	while (q.size() > 0) {
		...
		if (cur.SE > ans + G + R) break;

//		debug(cur);

		if (A[cur.FI] + G >= N) {
			chmn(ans, cur.SE + N - A[cur.FI]);
			continue;
		}

		for (set<int>::iterator it=adj[cur.FI].begin(); it!=adj[cur.FI].end(); it++) {
			if (V[*it] == false) {
				V[*it] = true;
				q.push(pll(*it, cur.SE + G + R));
			}
		}
	}
