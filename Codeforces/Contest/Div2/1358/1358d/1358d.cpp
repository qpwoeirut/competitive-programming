...
void solve() {
	...
	for (int i=0; i<N; i++) {
		cin >> A[i];
		days[i] = A[i] + (i>0 ? days[i-1] : 0);
		sum[i] = s(A[i]) + (i>0 ? sum[i-1] : 0);
	}
	debug1(sum, N);
	debug1(days, N);
	...
	int prev = 1;
	for (int i=0; i<N; i++) {
		while ((prev > i && days[N-1] - days[prev-1] + days[i] > X) || (prev < i && days[i] - days[prev-1] > X)) {
			prev = (prev + 1) % N;
		}
		...
		debug(ans);
		if (prev > i) {
			ans -= (sum[N-1] - sum[prev]);
			rem -= (days[N-1] - days[prev]);
		}
		else {
			ans -= sum[prev];
			rem -= days[N-1];
