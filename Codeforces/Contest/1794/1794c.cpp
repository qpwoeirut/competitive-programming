//1794c.cpp created at 03/04/23 09:33:55
...
void solve(int testcase) {
    ...
    multiset<int, greater<int>> usable;
    multiset<int> used;
    int d = 0;
    for (int i=0; i<N; ++i) {
        usable.ins(A[i]);
        while (usable.size() > 0 && *usable.begin() > d) {
            used.ins(*usable.begin());
            usable.erase(usable.begin());
            ++d;

            while (used.size() > 0 && *used.begin() < d) {
                usable.ins(*used.begin());
