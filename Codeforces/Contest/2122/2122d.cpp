//2122d.cpp created at 07/19/25 09:02:10
#include <bits/stdc++.h>
...
void solve(int testcase) {
    ...
    for (int i=0; i<N; ++i) {
        ...
        for (int j=0; j<N; ++j) {
            if (done[j]) continue;
            if (chmn(best, dist[j])) u = j;
        }
        assert(u != -1);

        if (adj[u].size() > 0) {
            for (int t=0; t<N; ++t) {
                ll v = adj[u][(t + best.fi) % adj[u].size()];
                chmn(dist[v], {dist[u].fi + t + 1, dist[u].se + t});
            }
        }
