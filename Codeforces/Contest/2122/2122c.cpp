//2122c.cpp created at 07/19/25 07:52:15
#include <bits/stdc++.h>
...
string to_string(const Point& p) {
    return "(i=" + to_string(p.idx) + ",x=" + to_string(p.x) + ",y=" + to_string(p.y) + ")";
}
...
ll brute() {
    ...
    do {
        ll dist = 0;
        for (int i=0; i<N; i+=2) {
            dist += abs(A[B[i]].x - A[B[i+1]].x) + abs(A[B[i]].y - A[B[i+1]].y);
        }
        chmx(best, dist);
