//1975d.cpp created at 05/25/24 08:01:37
...
void solve(int testcase) {
    ...
    debug1(cost, N);
    debug1(depth, N);
    const ll h_a = *max_element(depth, depth + N);
    const ll ans_a = cost[s_a] + depth[s_b] - h_a;

    fill(depth, depth + N, -1);
    dfs(s_b);
    debug1(cost, N);
    debug1(depth, N);
    const ll h_b = *max_element(depth, depth + N);
    const ll ans_b = cost[s_b] + depth[s_a] + (depth[s_a] % 2) - h_b;
