//1860c.cpp created at 08/17/23 07:34:56
...
void solve(int testcase) {
    ...
    ll ans = 0;
    for (int i=1; i<=N; ++i) {
        ll cur_ct = query(B[i], ct);
        ll cur_win = query(B[i], win);
        if (cur_ct == 0 || cur_win < cur_ct) {
            inc(B[i], win);
        } else ++ans;
        inc(B[i], ct);
        debug(i, B[i], cur_win, cur_ct);
    }
    cout << ans << '\n';
}
