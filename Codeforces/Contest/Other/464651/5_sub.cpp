...
namespace LargeSolution {
    const int MN = 100;
    const int TIME = 100;
    const int MX_TIME = 60;
    consteval array<int, TIME + 1> gen_p15() {
        array<int, TIME + 1> arr;
        for (int i=0; i<=TIME; ++i) {
            arr[i] = i * (int)(sqrt(i) + 1e-6);
    ...
    array<int, BOB_OPTIONS+1> bob_adj[MN][MN];
    array<int, BOB_OPTIONS+1> bob_tp[MN][MN];
    ...
    int expectimax(int a, int b, int t) {
        ...
        for (int target = 0; target < N; ++target) {
            ...
            const int prevent_escore = adj_escore - move_penalty - (100 - t) * p4(adj[a].size() + 1);
            cache[t][a][b] = max(cache[t][a][b], max(escore, prevent_escore));
        }
    ...
    void solve(int _n, int _m, const vector<pii>& edges, interactor interact) {
        ...
        for (int a=0; a<N; ++a) {
            for (int b=0; b<N; ++b) {
                ...
                sort(pos.rbegin(), pos.rend());
                while (pos.size() > BOB_OPTIONS + 1) pos.pop_back();
...
int main() {
    ...
    if (N <= 10) {
        SmallSolution::solve(N, M, edges, interact);
    } else {
        LargeSolution::solve(N, M, edges, interact);
    }
}
#endif
