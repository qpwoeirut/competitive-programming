//492161h.cpp created at 01/06/24 15:59:04
...
struct Box {
    ll k, q;
    int idx;
    Box(const int _idx, const ll _k, const ll _q): idx(_idx), k(_k), q(_q) {}
    const __int128 value() const {
        return p10[k] * q;
    }
    const bool operator<(const Box& o) const {
...
void solve(int testcase) {
    ...
    __int128 rem = 0;
    for (int d=0; d<MX; ++d) {
        for (const Box& b: box[d]) to_use.insert(b);
