...
int N, M;
inline const bool out(int r, int c) {
	return r <= 0 || r > N || c <= 0 || c > M;
}

inline const bool ok(int r, int c, int skip1, int skip2, int n) {
	if (out(r, c) || !mat[r][c]) return false;
	for (int i=0; i<n; i++) {
		if (i != skip1 && i != skip2) {
			if (mat[r+chr[i]][c+chc[i]]) return false;
		}
	}
	return true;
}

bool side_square(int r, int c, int len) {
	if (!ok(r, c, 1, 2, 4)) return false;
	if (mat[r-1][c-1]) return false;
	r++;
	for (int i=0; i+2<len; i++) {
		if (!ok(r, c, 0, 2, 4)) return false;
		r++;
	}
	if (!ok(r, c, 0, 1, 4)) return false;
	if (mat[r+1][c-1]) return false;
	c++;
	for (int i=0; i+2<len; i++) {
		if (!ok(r, c, 1, 3, 4)) return false;
		c++;
	}
	if (!ok(r, c, 0, 3, 4)) return false;
	if (mat[r+1][c+1]) return false;
	r--;
	for (int i=0; i+2<len; i++) {
		if (!ok(r, c, 0, 2, 4)) return false;
...
bool diag_square(int r, int c, int len) {
	if (!ok(r, c, 6, 7, 8)) return false;
	r++; c--;
	for (int i=0; i+2<len; i++) {
		if (!ok(r, c, 5, 6, 8)) return false;
		r++; c--;
	}
	if (!ok(r, c, 5, 7, 8)) return false;
	r++; c++;
	for (int i=0; i+2<len; i++) {
		if (!ok(r, c, 4, 7, 8)) return false;
		r++; c++;
	}
	if (!ok(r, c, 4, 5, 8)) return false;
	r--; c++;
	for (int i=0; i+2<len; i++) {
		if (!ok(r, c, 5, 6, 8)) return false;
		r--; c++;
	}
	if (!ok(r, c, 4, 6, 8)) return false;
	r--; c--;
	for (int i=0; i+2<len; i++) {
		if (!ok(r, c, 4, 7, 8)) return false;
		r--; c--;
		...
		for (int j=1; j<=M; j++) {
			if (mat[i][j]) {
				...
				for (int len=2; i+len-1<=N && j+len-1<=M; len++) {
					if (side_square(i, j, len)) {
						++ans;
						stop = true;
					}
					if (diag_square(i, j, len)) {
						++ans;
						stop = true;
					}
					if (stop) break;
