//496919f.cpp created at 01/07/24 15:43:33
...
void solve(int testcase) {
    ...
    for (int i=0; i<N; ++i) {
        for (int j=0; j<N; ++j) {
            cin >> G[i][j];
            A[i * N + j] = pll(G[i][j], i * N + j);
        }
    }

    sort(A, A+N*N, greater<pll>());
    vector<pll> ans;
    ll value = 0;
    for (int i=0; i<N*N; ++i) {
        const int row = root(A[i].se / N);
        const int col = root(A[i].se % N);
        debug(i, A[i], row, col);
        if (row == col) continue;
        if (elim[row] && elim[col]) continue;
        if (match[row][col]) continue;

        match[row][col] = true;
        match[col][row] = true;
        elim[max(row, col)] = true;
