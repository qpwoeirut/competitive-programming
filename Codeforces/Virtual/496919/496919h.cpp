//496919h.cpp created at 01/07/24 13:42:15
...
vector<int> rod[3];
vector<pii> ans;
void make_move(const int ia, const int ib) {
    assert(rod[ia].size() > 0);
    rod[ib].push_back(rod[ia].back());
    rod[ia].pop_back();
    ans.emplace_back(ia, ib);
}
bool try_move(const int ia, const int ib) {
    if (rod[ia].empty()) return false;
    if (rod[ib].empty() || rod[ia].back() < rod[ib].back()) {
        make_move(ia, ib);
        return true;
    }
    return false;
