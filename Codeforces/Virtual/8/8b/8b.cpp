...
set<pii> visited;
bool adj(pii pos, pii prev) {
	for (int i=0; i<4; i++) {
		if (pos.FI + chr[i] == prev.FI && pos.SE + chc[i] == prev.SE) continue;
		if (visited.find(pii(pos.FI + chr[i], pos.SE + chc[i])) != visited.end()) {
//			cout << pos.FI << " " << pos.SE << " " << prev.FI << " " << prev.SE << endl;
			return true;
		}
	}
	return false;
...
int main() {
	...
	pii cur(0,0);
	visited.INS(cur);
	pii prev(10000, 10000);

	bool opt = true;
	for (int i=0; i<s.size(); i++) {
		cur.FI += chr[moves[i]];
		cur.SE += chc[moves[i]];
//		cout << prev.FI << " " << prev.SE <<endl;
//		cout << cur.FI << " " << cur.SE << endl << endl;
		if (i>0 && (visited.find(cur) != visited.end() || adj(cur, prev))) {
			opt = false;
		}
		visited.INS(cur);
		prev = cur;
