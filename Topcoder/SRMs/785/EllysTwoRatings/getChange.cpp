	...
	EllysTwoRatings() {
		for (int i=0; i<MN; i++) {
			for (int j=0; j<=1000; j++) {
				dp[0][i][j] = dp[1][i][j] = 0.0;
			}
		}
	}
	void do_dp(int idx, int N, int start) {
		dp[idx][0][start] = 1.0;
		for (int i=1; i<=N; i++) {
			for (int j=1; j<=1000; j++) {
				double range = min(1000, j+100) - max(0, j-100) + 1;
				for (int k=max(0, j-100); k<=1000 && k<=j+100; k++) {
					if (idx) {
						dp[idx][i][k] += (dp[idx][i-1][j] - (dp[0][i-1][j] * dp[1][i-1][j])) / range;
					}
				}
			}
		}
	}
	double getChange(int N, int A, int B) {
		do_dp(0, N, A);
		do_dp(1, N, B);
		...
		for (int i=0; i<N; i++) {
			for (int j=1; j<=1000; j++) {
				never += dp[0][i][j] * dp[1][i][k];
				never += dp[0][i+1][j] * dp[1][i][k];
			}
		}
	}
};
...
int main() {
	EllysTwoRatings* obj = new EllysTwoRatings();
//	assert(obj->getChange() == 0);
//	assert(obj->getChange() == 0);
//	assert(obj->getChange() == 0);
//	assert(obj->getChange() == 0);
//	assert(obj->getChange() == 0);
