...
//			while (A[i] > 1 && seen.find(A[i]) != seen.end()) A[i]--;
//			seen.insert(A[i]);
//		}
//		return A;
		bool check;
		while (true) {
			...
			for (int i=A.size()-1; i>=0; i--) {
				if (A[i] <= 1) continue;
				if (read.find(A[i]) == read.end()) read.insert(A[i]);
				else {
					read.erase(A[i]);
					--A[i];
					check = true;
...
int main() {
	ValueDivision* obj = new ValueDivision();
	int tmp[7] = {1, 5, 7, 4, 5, 4, 1};
	cout << to_string(obj->getArray(vector<int>(tmp, tmp+7)));
	int a[7] = {7};
	cout << to_string(obj->getArray(vector<int>(a, a+1)));
	int b[7] = {7,4};
	cout << to_string(obj->getArray(vector<int>(b,b+2)));
	int c[7] = {7,7,7,7};
	cout << to_string(obj->getArray(vector<int>(c, c+4)));
