...
public:
	...
	long long findNumberOfSwaps(string S, int A0, int X, int Y, int N, int K) {
		ll A[MN];
		A[0] = A0;
		for (int i=1; i<N; i++) {
			A[i] = (A[i-1] * X + Y) % MOD;
...
int main() {
	SwapTheString* obj = new SwapTheString();
	assert(obj->findNumberOfSwaps("cbexa", 0, 0, 0, 5, 2) == 2);
	assert(obj->findNumberOfSwaps("", 5, 2, 3, 4, 1) == 3);
	assert(obj->findNumberOfSwaps("b", 1001, 1001, 1001, 5, 2) == 3);
	assert(obj->findNumberOfSwaps("", 999, 50000, 4797, 6, 3) == 2);
	assert(obj->findNumberOfSwaps("", 3435, 1000000000, 1812447358, 7, 2) == 5);
