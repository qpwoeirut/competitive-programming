...
struct Game {
    ...
    bool move(const Point& target) {
        ...
        if (target.x == 9267 && target.y == 3433) {
            cerr << "crouching: " << crouching << endl;
            cerr << guard[0][4][turn % guard[0][4].size()].a << ' ' << guard[0][4][turn % guard[0][4].size()].b << ' ' << guard[0][4][turn % guard[0][4].size()].c << endl;
            cerr << guard[1][4][turn % guard[1][4].size()].a << ' ' << guard[1][4][turn % guard[1][4].size()].b << ' ' << guard[1][4][turn % guard[1][4].size()].c << endl;
        }
...
Moves simulated_annealing(const Point& thief) {
    ...
    int order[POINTS];
    order[0] = 0;
    for (int i = 1; i <= G; ++i) order[i] = i;
    for (int i = 1; i <= C; ++i) order[G + i] = -i;
