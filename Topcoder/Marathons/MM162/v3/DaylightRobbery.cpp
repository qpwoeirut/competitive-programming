...
const int HEURISTIC_SPEED = CROUCH_SPEED;
const int MAX_SEARCH = 10000;
double heuristic(const double dist) {
    return ceil(dist / HEURISTIC_SPEED) + (dist / 40000) - 0.5;
}
...
// ofstream stats("stats.txt", ios_base::app);
...
int moves_to(const Game& game, const Point& target, const bool target_is_bribe, Moves& moves) {
    ...
    for (int i = 0; i < MAX_SEARCH && pq.size() > 0; ++i) {
        ...
        if ((target - cur.pos).sqnorm() <= CROUCH_SPEED * CROUCH_SPEED && !target_is_bribe && game.safe(cur.turn + 1, target, true)) {
            ...
            back_move.insert({nstate, cur});
            // stats << (game.thief - target).norm() << ' ' << cur.turn + 1 - game.turn << ' ' << i << ' ' << G << '\n';
            return recover_moves(back_move, nstate, moves);
        }
    ...
    // stats << (game.thief - target).norm() << ' ' << 1000 + pq.size() << ' ' << MAX_SEARCH << ' ' << G << '\n';
    return -1;
}

int search_moves(const Game& game, const int target, Moves& moves) {
    if (target == 0 || game.collected == C) return moves_out(game, moves);
...
int simulate(const Point& thief, const int order[], Moves& moves) {
    ...
    for (int i = 0; i < C + G + 1 && chrono::high_resolution_clock::now() < end_time; ++i) {
        int n_moves = search_moves(game, order[i], moves);
        if (n_moves == -1) {
            n_moves = moves_out(game, moves);
            if (n_moves == -1) return game.score;
        }
...
void construct_initial_order(const Point& thief, int order[]) {
    ...
    for (int i = 0; i < C; ++i) {
        ...
        for (int c = 0; c < C && c < 64; ++c) {
            if ((m_coin0 & (1ULL << c)) == 0 && dist > (coin[c] - cur).sqnorm()) {
                dist = (coin[c] - cur).sqnorm();
                closest = c;
            }
        }
        for (int c = 64; c < C; ++c) {
            if ((m_coin1 & (1ULL << (c - 64))) == 0 && dist > (coin[c] - cur).sqnorm()) {
                dist = (coin[c] - cur).sqnorm();
                closest = c;
            }
        }
        ...
        order[i] = -(closest + 1);
        if (closest < 64) {
            m_coin0 |= 1ULL << closest;
        } else {
            m_coin1 |= 1ULL << (closest - 64);
        }
        cur = coin[closest];
    }
    for (int i = 0; i < G; ++i) {
...
Moves simulated_annealing(const Point& thief) {
    ...
    int remaining;
    do {
        copy(order, order + C + G, new_order);
        ...
        if (cur_score > score || rng() < (1LL << 32) * temp) {
            cur_score = score;
            copy(new_order, new_order + C + G, order);
        }
    } while (remaining > 0);
