//tournament.cpp created at 04/26/25 18:11:22
#include <bits/stdc++.h>
...
void solve(int testcase) {
    ...
    for (; N > 1 && r<100; ++r) {
        const int r1 = (r&1)^1;
        for (int i=0; i<N; i+=2) {
            const auto p1 = A[r&1][i], p2 = A[r&1][i+1];
            if (p1.fi == p2.fi) {
                A[r1][i / 2] = make_pair(p1.fi + p2.fi, p1.se + p2.se);
            } else {
                A[r1][i / 2] = max(p1, p2);
                A[r1][i / 2].fi = p1.fi + p2.fi;
            }
        }
        N /= 2;
    }
