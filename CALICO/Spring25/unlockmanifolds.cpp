//unlockmanifolds.cpp created at 04/26/25 18:17:50
#include <bits/stdc++.h>
...
void solve(int testcase) {
    ...
    ll cur_r = 0, cur_c = 0;
    for (int i=1; i<=N*M; ++i) {
        ll dr = min(abs(A[i].fi - cur_r), min(abs(A[i].fi + N - cur_r), abs(cur_r + N - A[i].fi)));
        ll dc = min(abs(A[i].se - cur_c), min(abs(A[i].se + M - cur_c), abs(cur_c + M - A[i].se)));
