//kumi.cpp created at 04/26/25 18:57:15
#include <bits/stdc++.h>
...
void solve(int testcase) {
    ...
    while (total + 64 <= N && block[16] > 0) {
        --block[16];
        block[8] += 2;
        total += 64;
    }
    while (total + 16 <= N && block[8] > 0) {
        --block[8];
        block[4] += 2;
        total += 16;
    }
    while (total + 4 <= N && block[4] > 0) {
        --block[4];
        block[2] += 2;
        total += 4;
    }
    while (total + 1 <= N && block[2] > 0) {
        --block[2];
        block[1] += 1;
        total += 1;
    }
