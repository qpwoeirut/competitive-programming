//circle.cpp created at 04/26/25 17:06:24
#include <bits/stdc++.h>
...
double calculate(const double theta) {
    ...
    double lf = INF, ri = -INF;
    for (int i=0; i<N; ++i) {
        pt cur = A[i] * polar(1.0, theta);
...
void solve(int testcase) {
    ...
    for (int i=0; i<N; ++i) {
        for (int j=i+1; j<N; ++j) {
            double angle = arg(A[j] - A[i]);
            while (angle < 0) angle += PI;
            while (angle > Pu) angle -= PI;
            angs[M++] = angle;
        }
    }
    sort(angs, angs+M);
    ...
    ll most = -1;
    double most_ang = -1;
    for (const pair<const double, ll>& p: ct) {
        if (chmx(most, p.se)) {
            most_ang = p.fi;
        }
    }
    double area = calculate(-most_ang);
    debug(area, most_ang, most);
