//memorable_attractions.cpp created at 05/22/22 11:20:38
...
void solve(int testcase) {
    ...
    lo[0] = hi[0] = 0;
    for (int i=1; i<=N; ++i) {
        lo[i] = max(0LL, lo[i-1] - K);
        hi[i] = min(MX,  hi[i-1] + K);
    ...
    while (ch) {
        ch = false;
        for (int i=0; i<M; ++i) {
            ch |= chmx(lo[A[i]], lo[B[i]] - D[i]);
            ch |= chmn(hi[A[i]], hi[B[i]] + D[i]);

            ch |= chmx(lo[B[i]], lo[A[i]] - D[i]);
            ch |= chmn(hi[B[i]], hi[A[i]] + D[i]);
        }

        for (int i=1; i<=N; ++i) {
            ch |= chmx(lo[i], lo[i-1] - K);
            ch |= chmn(hi[i], hi[i-1] + K);
        }
        for (int i=N-1; i>=0; --i) {
            ch |= chmx(lo[i], lo[i+1] - K);
            ch |= chmn(hi[i], hi[i+1] + K);
