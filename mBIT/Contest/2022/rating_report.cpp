//rating_report.cpp created at 05/22/22 10:12:56
...
int dfs(int u, int target) {
    if (A[u] > 0)  return A[u] >= target ? 0 : -1;
    if (A[u] == 0) return 1;

    int ret = 0;
    if (A[u] == MIN) {
        for (const int v: adj[u]) {
            const int val = dfs(v, target);
            if (val == -1) {
                ret = -1;
                break;
            }
    ...
    debug(target, u, A[u], ret);

    return ret;
}

bool works(int ct, int target) {
    if (ct == -1) return false;
    for (; ct > 0; --ct, ++target) {
        while (target <= K && B[target]) ++target;
    }
    return target <= K + 1;
}
