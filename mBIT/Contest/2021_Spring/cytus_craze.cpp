//cytus_craze.cpp created at 06/12/21 07:27:58
...
void brute() {
    ...
    ll a = binpow(2, N - lg2(N), MOD);
    ll mx = binpow(2, lg2(N), MOD);
    //debug(ct[0], a, mx, K + 1, ans);
    assert(ans == a * min(mx, K + 1));
}

// https://oeis.org/A054243
void solve(int testcase) {
    ll a = binpow(2, N - lg2(N), MOD);
    ll mx = binpow(2, lg2(N), INF * 4);
    cout << (a * (min(mx, K + 1) % MOD)) % MOD << '\n';
}
...
int main() {
    ...
    //for (N=1; N<=20; ++N) { debug(N); for (K=1; K<=10000; ++K) { brute(); } }
