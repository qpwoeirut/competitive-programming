//future_gadget_lab.cpp created at 06/12/21 10:30:16
...
void dfs(int u, int par) {
    debug(u, par);
    if (par != -1) adj[u].erase(find(all(adj[u]), par));
    for (const int v: adj[u]) dfs(v, u);
}
...
void simulate() {
    int cur = 0;
    for (int j=0; j<J; ++j) {
        if (adj[cur].empty()) cur = 0;
        else {
            cur = adj[cur][rng() % sz(adj[cur])];
        }
    }
    ++ct[cur];
}
...
void solve(int testcase) {
    ...
    for (int i=0; i<N * 10000; ++i) simulate();
